...
fn header(head: Option<&str>) -> String {
    format!("# branch.oid {}\0# branch.head main\0", head.unwrap_or("(initial)"))
}
...
fn object_ids_and_branch_refs_reject_revision_and_option_injection() {
    ...
    for value in [ZERO, "HEAD", "--all", "1234567", "HEAD:path", "refs/heads/main"] {
        assert!(ObjectId::parse(value).is_err(), "{value}");
    }
    assert!(ObjectId::parse(&"A".repeat(40)).is_err());
    for value in [
        "--upload-pack=other", "main~1", "main^{tree}", "main:path", "a..b",
        "@{-1}", "a b", "a\nb", ".hidden", "a/.hidden", "a.lock", "a//b",
        "a/", "a.", "a\\b", "a*b", "a?b", "a[b", "HEAD", "@", "a\0b",
    ] {
    ...
    assert_eq!(resolve_ref_plan(&remote).args.last().unwrap(), "refs/remotes/origin/main^{commit}");
}
...
fn existing_refs_are_not_reinterpreted_as_branch_creation_input() {
    for name in ["-legacy", "HEAD", "@"] {
        ...
        assert_eq!(resolve_ref_plan(&reference).args.last().unwrap(), &format!("{full}^{{commit}}"));
        let status = format!("# branch.oid {A}\0# branch.head {name}\0");
        assert_eq!(parse_status(status.as_bytes()).unwrap().head.branch, Some(reference.clone()));
        let inventory = format!("worktree /repo\0HEAD {A}\0branch {full}\0\0");
        assert_eq!(parse_worktrees(inventory.as_bytes()).unwrap()[0].branch_ref.as_deref(), Some(full.as_str()));
        for create in [false, true] {
            assert!(worktree_add_plan("/linked", &reference, create, None).is_err());
        }
    }
    let limit = format!("refs/heads/{}", "x".repeat(MAX_PATH_BYTES - "refs/heads/".len()));
    assert!(GitRef::parse(&limit).is_ok());
    assert!(GitRef::parse(&format!("{limit}x")).is_err());
    for invalid in ["refs/heads/../other", "refs/heads/a\nb", "refs/heads/main^{commit}"] {
...
fn path_plans_preserve_literal_bytes_and_reject_escape_or_empty_selection() {
    let paths: Vec<_> = [
        "-force", ":(glob)*", "dir/space name", "line\nbreak", "utf8-\u{4e2d}",
        "back\\slash", "[ab]?*.txt", "tab\tname",
    ...
    for value in ["", "/root", "../outside", "a/../b", "a/./b", "a//b", "a/", ".git/config", "a\0b"] {
...
fn capture_rejects_even_record_boundary_truncation_and_unconfirmed_exit() {
    ...
    let good = CapturedOutput {
        stdout: bytes.as_bytes(), stderr: b"", exit_code: Some(0), timed_out: false,
        stdout_truncated: false, stderr_truncated: false,
    };
    let plan = status_plan();
    assert!(parse_status(plan.checked_stdout(good).unwrap()).unwrap().changes.is_empty());
    for bad in [
        CapturedOutput { stdout_truncated: true, ..good },
        CapturedOutput { stderr_truncated: true, ..good },
        CapturedOutput { timed_out: true, ..good },
        CapturedOutput { exit_code: None, ..good },
        CapturedOutput { exit_code: Some(128), ..good },
    ] {
        assert!(plan.checked_stdout(bad).is_err());
    }
    let large = vec![0; MAX_LIST_BYTES + 1];
    assert!(plan.checked_stdout(CapturedOutput { stdout: &large, ..good }).is_err());
    let stderr = vec![0; DIAGNOSTIC_BYTES + 1];
    assert!(plan.checked_stdout(CapturedOutput { stderr: &stderr, ..good }).is_err());
}
...
fn literal_pathspecs_stay_global_without_changing_unrelated_hooks() {
    ...
    let DiscardPlan::Git(discard) = discard_plan(&path, Some(&head), true).unwrap() else { panic!() };
    for (plan, expected_path) in [
        (stage_plan(paths).unwrap(), path.as_str()), (stage_all_plan(), "."),
        ...
        (index_entry_plan(&path).unwrap(), path.as_str()), (discard, path.as_str()),
    ] {
        let literal = plan.args.iter().position(|arg| arg == "--literal-pathspecs").unwrap();
        let subcommand = plan.args.iter().position(|arg| !arg.starts_with('-')).unwrap();
        let separator = plan.args.iter().position(|arg| arg == "--").unwrap();
        assert!(literal < subcommand && subcommand < separator, "{:?}", plan.args);
        assert_eq!(plan.args.last().unwrap(), expected_path);
    }
    let branch = GitRef::local("feature").unwrap();
    for plan in [
        commit_plan("commit").unwrap(), pull_plan(), push_plan(),
        worktree_add_plan("/linked", &branch, true, Some(&head)).unwrap(),
        worktree_remove_plan("/linked", "/main", false).unwrap(), worktree_prune_plan(),
    ] {
        assert!(!plan.args.iter().any(|arg| arg == "--literal-pathspecs"));
    }
}
...
fn status_preserves_partial_index_rename_conflict_untracked_and_ignored_semantics() {
    let bytes = format!(
        "{}1 MM N... 100644 100644 100644 {A} {B} partial\0\
         ...
         ? :(glob)*\0! ignored/\0", header(Some(A))
    );
    ...
    assert_eq!(parsed.changes[2].unstaged_status, Some(GitStatus::Conflicted));
    assert_eq!(parsed.changes[3].unstaged_status, Some(GitStatus::Deleted));
    assert_eq!(parsed.changes[5].path, ":(glob)*");
    assert_eq!(parsed.changes[5].unstaged_status, Some(GitStatus::Untracked));
}
...
fn status_distinguishes_unborn_detached_sha256_and_intent_to_add() {
    ...
    assert!(parse_status(detached.as_bytes()).unwrap().head.branch.is_none());
    let sha256 = "a".repeat(64);
    assert_eq!(parse_status(header(Some(&sha256)).as_bytes()).unwrap().head.oid, Some(oid(&sha256)));
    let intent = format!("{}1 .A N... 000000 000000 100644 {ZERO} {ZERO} intent\0", header(Some(A)));
    assert_eq!(parse_status(intent.as_bytes()).unwrap().changes[0].unstaged_status, Some(GitStatus::Added));
}
...
fn status_preserves_untracked_directories_outside_actionable_file_rows() {
    ...
    assert_eq!(status.changes[1].unstaged_status, Some(GitStatus::Untracked));
    ...
    assert_eq!(&all.args[all.args.len() - 4..], &["add", "--all", "--", "."]);
}

#[test]
fn status_rejects_malformed_and_duplicate_directory_paths() {
    for records in [
        "? /\0", "? //\0", "? /absolute/\0", "? nested//\0", "? nested//child/\0",
        "? ../outside/\0", "? nested/./child/\0", "? nested/../child/\0",
        "? .git/\0", "? nested/.git/\0", "? nested/",
        "? nested/\0? nested/\0", "? nested/\0? nested\0", "? nested\0? nested/\0",
    ] {
        assert!(parse_status(format!("{}{records}", header(Some(A))).as_bytes()).is_err(), "{records:?}");
    }
    let tracked = format!("1 .M N... 100644 100644 100644 {A} {A} nested\0");
    for records in [format!("{tracked}? nested/\0"), format!("? nested/\0{tracked}")] {
...
fn status_rejects_malformed_truncated_non_utf8_and_duplicate_records() {
    for bytes in [
        Vec::new(), b"? file\0".to_vec(), header(Some(A)).into_bytes()[..10].to_vec(),
        format!("{}? file", header(Some(A))).into_bytes(),
        format!("{}? file\0? file\0", header(Some(A))).into_bytes(),
        format!("{}2 R. N... 100644 100644 100644 {A} {B} R100 new\0", header(Some(A))).into_bytes(),
        format!("{}2 R. N... 100644 100644 100644 {A} {B} \u{4e2d} new\0old\0", header(Some(A))).into_bytes(),
        format!("{}1 .Z N... 100644 100644 100644 {A} {B} path\0", header(Some(A))).into_bytes(),
        format!("{}1 .. N... 100644 100644 100644 {A} {B} path\0", header(Some(A))).into_bytes(),
        format!("{}u ZZ N... 100644 100644 100644 100644 {A} {B} {C} path\0", header(Some(A))).into_bytes(),
...
fn repository_captures_reject_mixed_object_formats_even_without_head() {
    ...
    let status = format!(
        "{}1 A. N... 000000 100644 100644 {ZERO} {A} first\0\
         1 A. N... 000000 100644 100644 {zero256} {sha256} second\0", header(None)
    );
    ...
    assert!(parse_branches(format!("refs/heads/b\0{sha256}\0\0\n").as_bytes(), Some(&oid(A))).is_err());
...
fn history_has_length_framing_all_parents_and_no_implicit_head_at_end() {
    ...
    assert!(!plan.args.iter().any(|arg| arg.contains("skip") || arg == "--first-parent"));
    assert!(log_plan(&[], 30).unwrap().is_none());
    assert!(log_plan(&[oid(A)], 0).is_err());
    assert!(log_plan(&[oid(A)], MAX_LOG_COMMITS + 1).is_err());
    assert_eq!(parse_commit_parents(format!("{A} {B} {C}\n").as_bytes(), &oid(A)).unwrap(), [oid(B), oid(C)]);
...
fn history_rejects_partial_payloads_and_embedded_record_injection() {
    ...
    let injected = log_record(A, B, "subject", &format!("body\0{C}\0\0Author\x001\0forged\0body"));
...
fn commit_files_and_exact_object_lookups_preserve_paths_and_missing_sides() {
    let files = parse_commit_files(b"A\0space name\0D\0removed\0R100\0old\nname\0new\nname\0").unwrap();
    ...
    assert_eq!(parse_tree_entry(gitlink.as_bytes(), "module").unwrap().unwrap().kind, EntryKind::Commit);
    let index = format!("100644 {A} 0\t:(glob)*\0");
    assert_eq!(parse_index_entry(index.as_bytes(), ":(glob)*").unwrap().unwrap().oid, oid(A));
...
fn exact_lookup_absence_requires_complete_success_and_single_matching_path() {
    let empty = CapturedOutput {
        stdout: b"", stderr: b"", exit_code: Some(0), timed_out: false,
        stdout_truncated: false, stderr_truncated: false,
    };
    for plan in [tree_entry_plan(&oid(A), "file").unwrap(), index_entry_plan("file").unwrap()] {
        assert!(plan.checked_stdout(empty).unwrap().is_empty());
        for invalid in [
            CapturedOutput { exit_code: Some(128), ..empty },
            CapturedOutput { stdout_truncated: true, ..empty },
            CapturedOutput { stderr_truncated: true, ..empty },
            CapturedOutput { timed_out: true, ..empty },
            CapturedOutput { exit_code: None, ..empty },
        ] {
    ...
    assert_eq!(parse_tree_entry(tree.as_bytes(), "file").unwrap().unwrap().kind, EntryKind::Tree);
    for bad in [
...
fn diff_plans_compare_both_modes_to_head_and_commit_to_first_parent() {
    ...
    assert_eq!(working.old, BlobLookup::Tree { commit: oid(A), path: "old".to_string() });
    assert_eq!(working.new, BlobLookup::Worktree { path: "new".to_string() });
    assert_eq!(staged.new, BlobLookup::Index { path: "new".to_string() });
    assert_eq!(working_diff_plan(None, "new", None, false).unwrap().old, BlobLookup::Empty);
    assert_eq!(commit_diff_plan(&oid(B), Some(&oid(A)), "new", Some("old")).unwrap().old, staged.old);
    assert_eq!(commit_diff_plan(&oid(A), None, "new", None).unwrap().old, BlobLookup::Empty);
...
fn mutations_have_explicit_unborn_discard_and_host_configuration_semantics() {
    ...
    assert!(unstage_all_plan(None).args.iter().any(|arg| arg == "--empty"));
    let born = unstage_all_plan(Some(&oid(A)));
    assert_eq!(&born.args[born.args.len() - 4..], &["reset", "--mixed", "HEAD", "--"]);
    assert!(!born.args.iter().any(|arg| arg == "--hard" || arg == A));
    assert!(matches!(discard_plan("new", None, false).unwrap(), DiscardPlan::RemoveUntrackedFile { .. }));
    let DiscardPlan::Git(discard) = discard_plan("tracked", Some(&oid(A)), true).unwrap() else { panic!() };
    ...
    assert_eq!(&commit.args[commit.args.len() - 3..], &["commit", "-m", message]);
    assert!(commit_plan(" \n").is_err());
    assert!(commit_plan("nul\0").is_err());
    for plan in [commit, pull_plan(), push_plan()] {
        assert_eq!(plan.effect, CommandEffect::Mutation);
        assert!(!plan.args.iter().any(|arg| arg == "--no-verify" || arg == "--force" || arg == "-c"));
    }
}

#[test]
fn worktree_plans_reuse_posix_inventory_and_exclude_main_removal() {
    let bytes = format!("worktree /srv/Repo\0HEAD {A}\0branch refs/heads/main\0\0worktree /srv/repo\0HEAD {B}\0detached\0\0");
    ...
    assert_eq!(&create.args[create.args.len() - 5..], &["-b", "feature", "--", "/srv/new\nworktree", A]);
    assert!(worktree_add_plan("relative", &branch, true, None).is_err());
    assert!(worktree_add_plan("/srv/new", &GitRef::remote("origin/main").unwrap(), false, None).is_err());
    ...
    assert_eq!(&remove.args[remove.args.len() - 3..], &["--force", "--", "/srv/linked"]);
}
...
fn worktree_records_require_head_or_bare_without_erasing_prunable_rows() {
    ...
    assert_eq!(parse_worktrees(unborn.as_bytes()).unwrap()[0].head.as_deref(), Some(ZERO));
    let prunable = format!(
...
mod actions_fixtures {
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            let root = std::env::temp_dir().join(format!(
                "mini-term-git-cli-{}-{nonce}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)
            ));
        ...
        fn output_at_with_date(&self, cwd: &Path, args: &[String], date: &str) -> Output {
            ...
            command.env_clear()
                ...
                .env("LC_ALL", "C")
                .args(args).current_dir(cwd).stdin(Stdio::null());
        ...
        fn run_at(&self, cwd: &Path, plan: &GitCommand) -> Vec<u8> {
            ...
            plan.checked_stdout(CapturedOutput {
                stdout: &output.stdout, stderr: &output.stderr, exit_code: output.status.code(),
                timed_out: false, stdout_truncated: false, stderr_truncated: false,
            }).unwrap_or_else(|error| panic!("{error}: {}", String::from_utf8_lossy(&output.stderr))).to_vec()
        }
        ...
        fn content(&self, lookup: &BlobLookup) -> Option<Vec<u8>> {
            let oid = match lookup {
                ...
                BlobLookup::Tree { commit, path } => {
                    let entry = parse_tree_entry(&self.run(&tree_entry_plan(commit, path).unwrap()), path).unwrap()?;
                    assert_eq!(entry.kind, EntryKind::Blob);
                    assert!(entry.size.unwrap() <= MAX_BLOB_BYTES as u64);
                    entry.oid
                }
                BlobLookup::Index { path } => {
                    parse_index_entry(&self.run(&index_entry_plan(path).unwrap()), path).unwrap()?.oid
                }
            ...
            assert!(parse_blob_size(&self.run(&blob_size_plan(&oid))).unwrap() <= MAX_BLOB_BYTES as u64);
        ...
        fn diff(&self, plan: &DiffPlan) -> GitDiffResult {
            ...
            build_diff(
                old.as_deref().map_or(DiffContent::Missing, DiffContent::Bytes),
                new.as_deref().map_or(DiffContent::Missing, DiffContent::Bytes),
            )
        }
    }
    ...
    fn actual_plans_preserve_unborn_hostile_filenames_and_index_only_unstage() {
        ...
        let paths: Vec<_> = ["space name", "line\nbreak", "utf8-\u{4e2d}", "-leading", ":(glob)*", "[a]?", "back\\slash", "tab\tfile", "nested/dir/file"]
            .into_iter().map(str::to_string).collect();
        for path in &paths {
            fixture.file(path, b"original\n");
        }
        fixture.run(&stage_plan(&[":(glob)*".to_string()]).unwrap());
        assert_eq!(fixture.status().changes.iter().filter(|entry| entry.staged_status.is_some()).count(), 1);
        fixture.run(&stage_all_plan());
        let status = fixture.status();
        assert_eq!(status.changes.len(), paths.len());
        assert!(status.changes.iter().all(|entry| entry.staged_status == Some(GitStatus::Added)));
        fixture.run(&unstage_plan(None, &[":(glob)*".to_string()]).unwrap());
        assert!(parse_index_entry(&fixture.run(&index_entry_plan(":(glob)*").unwrap()), ":(glob)*").unwrap().is_none());
        assert_eq!(std::fs::read(fixture.repo.join(":(glob)*")).unwrap(), b"original\n");
        fixture.run(&unstage_all_plan(None));
        assert!(fixture.status().changes.iter().all(|entry| entry.staged_status.is_none()));
        fixture.file("unborn-discard", b"unborn\n");
        fixture.run(&stage_plan(&["unborn-discard".to_string()]).unwrap());
        let DiscardPlan::Git(discard) = discard_plan("unborn-discard", None, true).unwrap() else { panic!() };
        fixture.run(&discard);
        assert!(!fixture.repo.join("unborn-discard").exists());
        fixture.file(".gitignore", b"ignored\n");
        fixture.file("ignored", b"sentinel\n");
        fixture.run(&stage_all_plan());
        assert!(!fixture.status().changes.iter().any(|entry| entry.path == "ignored"));
    ...
    fn actual_nested_repository_status_retains_modified_parent_and_explicit_stage_all() {
        ...
        assert!(all.changes.iter().any(|row| row.path == "nested" && row.staged_status == Some(GitStatus::Added)));
        assert!(all.changes.iter().any(|row| row.path == "parent" && row.staged_status == Some(GitStatus::Modified)));
        assert_eq!(std::fs::read(nested.join("child")).unwrap(), b"nested contents\n");
    }
    ...
    fn actual_existing_refs_remain_qualified_through_read_plans() {
        ...
        let branches = parse_branches(&fixture.run(&branches_plan()), fixture.status().head.oid.as_ref()).unwrap();
        for name in ["-legacy", "HEAD", "@"] {
            ...
            let history = parse_log(&fixture.run(&log_plan(&[resolved], 30).unwrap().unwrap())).unwrap();
            assert_eq!(history.len(), 1);
            assert_eq!(history[0].hash, head.as_str());
        }
        fixture.raw(&["symbolic-ref", "HEAD", "refs/heads/-legacy"]);
        assert_eq!(fixture.status().head.branch.unwrap().as_str(), "refs/heads/-legacy");
        let inventory = parse_worktrees(&fixture.run(&worktree_list_plan())).unwrap();
        assert_eq!(inventory[0].branch_ref.as_deref(), Some("refs/heads/-legacy"));
    }
    ...
    fn actual_commit_hook_keeps_its_own_git_pathspec_semantics() {
        ...
        let head = fixture.status().head.oid.unwrap();
        let diff = fixture.diff(&commit_diff_plan(&head, Some(&root), "generated-one.txt", None).unwrap());
    ...
    fn actual_sha256_status_history_and_blob_plans_use_full_ids() {
        ...
        fixture.raw_at(&fixture.root, &[
            "init", "--object-format=sha256", "--initial-branch=main", repo.to_str().unwrap(),
        ]);
        ...
        let head = parse_status(&fixture.run_at(&repo, &status_plan())).unwrap().head.oid.unwrap();
        assert_eq!(head.as_str().len(), 64);
        assert!(parse_commit_parents(&fixture.run_at(&repo, &commit_parents_plan(&head)), &head).unwrap().is_empty());
        let log = parse_log(&fixture.run_at(&repo, &log_plan(std::slice::from_ref(&head), 30).unwrap().unwrap())).unwrap();
        assert_eq!(log[0].hash, head.as_str());
        let branches = parse_branches(&fixture.run_at(&repo, &branches_plan()), Some(&head)).unwrap();
        assert_eq!(branches[0].commit_hash, head.as_str());
        let files = parse_commit_files(&fixture.run_at(&repo, &commit_files_plan(&head, None))).unwrap();
        assert_eq!(files[0].path, path);
        let tree = parse_tree_entry(&fixture.run_at(&repo, &tree_entry_plan(&head, path).unwrap()), path).unwrap().unwrap();
        let index = parse_index_entry(&fixture.run_at(&repo, &index_entry_plan(path).unwrap()), path).unwrap().unwrap();
        assert_eq!(tree.oid, index.oid);
        assert_eq!(tree.oid.as_str().len(), 64);
        assert_eq!(parse_blob_size(&fixture.run_at(&repo, &blob_size_plan(&tree.oid))).unwrap(), 7);
    ...
    fn actual_partial_status_diff_discard_and_index_lock_match_safety_contract() {
        ...
        assert_eq!(serde_json::to_value(status.changes).unwrap(), serde_json::to_value(local).unwrap());
        for staged in [false, true] {
            let actual = fixture.diff(&working_diff_plan(Some(&head), "tracked", None, staged).unwrap());
            let local = crate::git::get_git_diff(&fixture.repo, "tracked", Some(staged)).unwrap();
            assert_eq!(actual.old_content, "HEAD\n");
            assert_eq!(actual.new_content, if staged { "INDEX\n" } else { "WORKTREE\n" });
            assert_eq!(serde_json::to_value(actual).unwrap(), serde_json::to_value(local).unwrap());
        }
        fixture.run(&unstage_plan(Some(&head), &["tracked".to_string()]).unwrap());
        assert!(fixture.status().changes[0].staged_status.is_none());
        assert_eq!(std::fs::read(fixture.repo.join("tracked")).unwrap(), b"WORKTREE\n");
        fixture.run(&stage_plan(&["tracked".to_string()]).unwrap());
        let DiscardPlan::Git(discard) = discard_plan("tracked", Some(&head), true).unwrap() else { panic!() };
        fixture.run(&discard);
        assert!(fixture.status().changes.is_empty());
        assert_eq!(std::fs::read(fixture.repo.join("tracked")).unwrap(), b"HEAD\n");
        assert_eq!(std::fs::read(fixture.repo.join("sibling")).unwrap(), b"safe\n");
        ...
        fixture.run(&stage_plan(&["tracked".to_string()]).unwrap());
        assert_eq!(fixture.status().changes[0].staged_status, Some(GitStatus::Deleted));
        ...
        let output = fixture.output_at(&fixture.repo, &stage_plan(&["tracked".to_string()]).unwrap().args);
        assert!(!output.status.success());
        assert_eq!(std::fs::read(fixture.repo.join(".git/index.lock")).unwrap(), b"external lock");
    }
    ...
    fn actual_history_filters_without_checkout_and_continues_from_both_merge_parents() {
        ...
        let parents = parse_commit_parents(&fixture.run(&commit_parents_plan(&merge)), &merge).unwrap();
        assert_eq!(parents, [main.clone(), side.clone()]);
        let first = parse_log(&fixture.run(&log_plan(std::slice::from_ref(&merge), 1).unwrap().unwrap())).unwrap();
        assert_eq!(first[0].hash, merge.as_str());
        assert_eq!(first[0].parent_hashes, [main.as_str(), side.as_str()]);
        let continuation = parse_log(&fixture.run(&log_plan(&parents, 30).unwrap().unwrap())).unwrap();
        let hashes: std::collections::HashSet<_> = continuation.iter().map(|commit| commit.hash.as_str()).collect();
        assert_eq!(hashes, [main.as_str(), side.as_str(), root.as_str()].into_iter().collect());
        let refs = parse_branches(&fixture.run(&branches_plan()), Some(&merge)).unwrap();
        let side_ref = GitRef::from_branch(refs.iter().find(|branch| branch.name == "side").unwrap()).unwrap();
        let side_tip = parse_object_id(&fixture.run(&resolve_ref_plan(&side_ref))).unwrap();
        let filtered = parse_log(&fixture.run(&log_plan(&[side_tip], 30).unwrap().unwrap())).unwrap();
        assert_eq!(filtered.len(), 2);
        let local = crate::git::get_git_log(&fixture.repo, None, Some(30), Some("side")).unwrap();
        assert_eq!(serde_json::to_value(&filtered).unwrap(), serde_json::to_value(local).unwrap());
        assert_eq!(fixture.status().head.oid, Some(merge.clone()));
        let files = parse_commit_files(&fixture.run(&commit_files_plan(&merge, parents.first()))).unwrap();
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, "side");
        let root_log = filtered.iter().find(|commit| commit.hash == root.as_str()).unwrap();
    ...
    fn actual_clock_skewed_history_keeps_date_order_and_all_parent_continuations() {
        ...
        let tree = parse_object_id(&fixture.raw(&["rev-parse", &format!("{}^{{tree}}", initial.as_str())])).unwrap();
        let commit_at = |message: &str, date: &str, parents: &[&ObjectId]| {
            let mut args = vec!["commit-tree".to_string(), tree.as_str().to_string(), "-m".to_string(), message.to_string()];
        ...
        let log = parse_log(&fixture.run(&log_plan(std::slice::from_ref(&merge), 30).unwrap().unwrap())).unwrap();
        let hashes: Vec<_> = log.iter().map(|commit| commit.hash.as_str()).collect();
        assert_eq!(hashes, [merge.as_str(), right.as_str(), left.as_str(), root.as_str()]);
        let local = crate::git::get_git_log(&fixture.repo, None, Some(30), None).unwrap();
        assert_eq!(serde_json::to_value(&log).unwrap(), serde_json::to_value(local).unwrap());
        let first = parse_log(&fixture.run(&log_plan(&[merge], 1).unwrap().unwrap())).unwrap();
        let tips: Vec<_> = first[0].parent_hashes.iter().map(|hash| oid(hash)).collect();
        let continuation = parse_log(&fixture.run(&log_plan(&tips, 30).unwrap().unwrap())).unwrap();
        let continued: Vec<_> = continuation.iter().map(|commit| commit.hash.as_str()).collect();
    ...
    fn actual_rename_binary_oversized_and_conflicted_entries_are_explicit() {
        ...
        let root = fixture.commit("root");
        std::fs::rename(fixture.repo.join("old name"), fixture.repo.join("new\nname")).unwrap();
        fixture.run(&stage_all_plan());
        let rename = fixture.status().changes.into_iter().find(|entry| entry.path == "new\nname").unwrap();
        assert_eq!(rename.old_path.as_deref(), Some("old name"));
        fixture.run(&commit_plan("rename").unwrap());
        let head = fixture.status().head.oid.unwrap();
        let files = parse_commit_files(&fixture.run(&commit_files_plan(&head, Some(&root)))).unwrap();
        assert_eq!(files[0].old_path.as_deref(), Some("old name"));
        let diff = fixture.diff(&commit_diff_plan(&head, Some(&root), "new\nname", Some("old name")).unwrap());
        let local = crate::git::get_commit_file_diff(&fixture.repo, head.as_str(), "new\nname", Some("old name")).unwrap();
        assert_eq!(serde_json::to_value(diff).unwrap(), serde_json::to_value(local).unwrap());
        assert!(fixture.diff(&commit_diff_plan(&root, None, "binary", None).unwrap()).is_binary);
        let large = parse_tree_entry(&fixture.run(&tree_entry_plan(&root, "oversized").unwrap()), "oversized").unwrap().unwrap();
        ...
        let merge = fixture.output_at(&fixture.repo, &["merge".to_string(), "--no-ff".to_string(), "conflicting-side".to_string()]);
        assert!(!merge.status.success());
        let status = fixture.status();
        let conflict = status.changes.iter().find(|entry| entry.path == "conflict").unwrap();
        assert_eq!(conflict.staged_status, Some(GitStatus::Conflicted));
        assert_eq!(conflict.unstaged_status, Some(GitStatus::Conflicted));
        assert!(parse_index_entry(&fixture.run(&index_entry_plan("conflict").unwrap()), "conflict").is_err());
    ...
    fn actual_worktree_authority_create_remove_force_and_prune_plans() {
        ...
        let authority = parse_repository_authority(&fixture.run(&plans[0]), &fixture.run(&plans[1]), &fixture.run(&plans[2])).unwrap();
        assert!(!authority.is_linked_worktree());
        let target = fixture.root.join("linked\nworktree");
        let target_str = target.to_str().unwrap();
        fixture.run(&worktree_add_plan(target_str, &GitRef::local("feature").unwrap(), true, Some(&head)).unwrap());
        let linked = parse_repository_authority(&fixture.run_at(&target, &plans[0]), &fixture.run_at(&target, &plans[1]), &fixture.run_at(&target, &plans[2])).unwrap();
        ...
        assert!(!fixture.output_at(&fixture.repo, &remove.args).status.success());
        assert!(target.exists());
        fixture.run(&worktree_remove_plan(target_str, &authority.worktree_root, true).unwrap());
        assert!(!target.exists());
        assert_eq!(std::fs::read(fixture.repo.join("tracked")).unwrap(), b"main sentinel\n");
        fixture.run(&worktree_add_plan(target_str, &GitRef::local("feature").unwrap(), false, None).unwrap());
        std::fs::remove_dir_all(&target).unwrap();
        fixture.run(&worktree_prune_plan());
        assert_eq!(parse_worktrees(&fixture.run(&worktree_list_plan())).unwrap().len(), 1);
    }
    ...
    fn actual_pull_push_plans_use_fixture_host_git_configuration() {
        ...
        assert_eq!(parse_object_id(&fixture.raw(&["--git-dir", origin, "rev-parse", "refs/heads/main"])).unwrap(), pushed);
        ...
        assert_eq!(std::fs::read(fixture.repo.join("tracked")).unwrap(), b"pulled\n");
        assert!(fixture.status().changes.is_empty());
    }
    ...
    fn actual_non_utf8_paths_and_symlink_blobs_do_not_alias_files() {
        ...
        let fixture = Fixture::new();
        let invalid = fixture.repo.join(std::ffi::OsString::from_vec(b"non-utf8-\xff".to_vec()));
        ...
        assert_eq!(std::fs::read(fixture.root.join("sentinel")).unwrap(), b"do not follow\n");
    }
}
