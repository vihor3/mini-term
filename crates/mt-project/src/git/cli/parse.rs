...
use super::{
    EntryKind, GitRef, HeadState, IndexEntry, MAX_LIST_BYTES, MAX_LOG_COMMITS,
    MAX_PATH_BYTES, MAX_RECORDS, MAX_WORKTREES, ObjectId, RepositoryAuthority, RepositoryStatus,
    TreeEntry, validate_absolute_path, validate_oid, validate_repo_path,
};
...
fn nul_fields(bytes: &[u8], limit: usize) -> Result<Vec<&[u8]>> {
    ...
    let body = bytes.strip_suffix(b"\0").ok_or_else(|| anyhow!("Incomplete NUL framing"))?;
...
fn mode(value: &str) -> Result<u32> {
    ...
    let mode = u32::from_str_radix(value, 8)?;
        ...
        matches!(mode, 0 | 0o040000 | 0o100644 | 0o100755 | 0o120000 | 0o160000),
...
fn decimal(value: &str) -> Result<u64> {
    ensure!(!value.is_empty() && value.bytes().all(|b| b.is_ascii_digit()), "Invalid Git number");
    Ok(value.parse()?)
}
...
fn similarity(value: &str) -> Result<u8> {
    ...
    ensure!(score.len() <= 3 && decimal(score)? <= 100, "Invalid Git similarity score");
    Ok(kind)
}
...
pub fn parse_status(bytes: &[u8]) -> Result<RepositoryStatus> {
    ...
    while let Some(record) = records.next() {
        ...
        if let Some(header) = record.strip_prefix("# ") {
            ...
            if let Some(value) = header.strip_prefix("branch.oid ") {
                ...
                oid = Some(if value == "(initial)" { None } else { Some(ObjectId::parse(value)?) });
                if value != "(initial)" {
                ...
                branch = Some(if value == "(detached)" { None } else {
        ...
        saw_entries = true;
        let head = oid.as_ref().ok_or_else(|| anyhow!("Missing Git status HEAD header"))?;
        ...
        let (file_path, old_path, staged_status, unstaged_status) =
            if let Some(value) = record.strip_prefix("? ") {
                if let Some(directory) = value.strip_suffix('/') {
                    ensure!(value.len() <= MAX_PATH_BYTES, "Git directory path exceeds its limit");
                    validate_repo_path(directory)?;
                    ensure!(paths.insert(directory.to_string()), "Duplicate Git status path");
                    ensure!(paths.len() <= MAX_RECORDS, "Too many Git status entries");
                    untracked_directories.push(value.to_string());
                    continue;
                }
                (path(value.as_bytes())?, None, None, Some(if head.is_none() {
                    GitStatus::Added
                } else {
                    GitStatus::Untracked
                }))
                ...
                let kind = record.as_bytes().first().copied().ok_or_else(|| anyhow!("Empty Git status record"))?;
                ...
                ensure!(fields.len() == count && fields[0].len() == 1, "Incomplete Git status record");
                ...
                if kind == b'u' {
                    ...
                    (path(fields[10].as_bytes())?, None, Some(GitStatus::Conflicted), Some(GitStatus::Conflicted))
                } else {
                    ...
                    ensure!(staged.is_some() || unstaged.is_some(), "Unchanged Git status entry");
                    let old_path = if kind == b'2' {
                        ensure!(!fields[8].is_empty(), "Missing Git rename score");
                        let rename_kind = similarity(fields[8])?;
                        ensure!(xy.contains(&rename_kind), "Git rename status/score mismatch");
                        Some(path(records.next().ok_or_else(|| anyhow!("Missing Git rename source"))?)?)
                    } else {
                        ensure!(!xy.iter().any(|c| matches!(c, b'R' | b'C')), "Git rename is missing source framing");
                        None
                    };
                    (path(fields[count - 1].as_bytes())?, old_path, staged, unstaged)
                }
            };
        ensure!(paths.insert(file_path.clone()), "Duplicate Git status path");
        ensure!(old_path.as_deref() != Some(file_path.as_str()), "Git rename source equals destination");
        ensure!(paths.len() <= MAX_RECORDS, "Too many Git status entries");
        let label = staged_status.as_ref().or(unstaged_status.as_ref())
            .map(crate::git::status_label).unwrap_or("").to_string();
    ...
    ensure!(head.oid.is_some() || head.branch.is_some(), "Unborn HEAD cannot be detached");
...
fn parents(value: &str, commit: &ObjectId) -> Result<Vec<ObjectId>> {
    ...
    for value in value.split(' ') {
        ensure!(result.len() < MAX_LOG_COMMITS, "Too many Git commit parents");
        let parent = ObjectId::parse(value)?;
        ensure!(parent.as_str().len() == commit.as_str().len(), "Mixed Git object ID formats");
...
pub fn parse_log(bytes: &[u8]) -> Result<Vec<GitCommitInfo>> {
    ...
    while !remaining.is_empty() {
        ...
        let header_end = remaining.iter().take(32).position(|byte| *byte == b'\n')
            .ok_or_else(|| anyhow!("Missing Git log size header"))?;
        let size = utf8(&remaining[..header_end])?.strip_prefix("log size ")
        ...
        let parent_hashes = parents(utf8(fields[1])?, &hash)?.into_iter()
            .map(|oid| oid.as_str().to_string()).collect();
...
pub fn parse_commit_parents(bytes: &[u8], commit: &ObjectId) -> Result<Vec<ObjectId>> {
    ...
    ensure!(ObjectId::parse(oid)? == *commit, "Git parent result belongs to another commit");
    parents(rest, commit)
}
...
pub fn parse_commit_files(bytes: &[u8]) -> Result<Vec<CommitFileInfo>> {
    ...
    while let Some(status) = fields.next() {
        ...
        let first_path = path(fields.next().ok_or_else(|| anyhow!("Missing Git diff path"))?)?;
        let (status, old_path, file_path) = match status {
            ...
            value if value.starts_with(['R', 'C']) => {
                ...
                let destination = path(fields.next().ok_or_else(|| anyhow!("Missing Git diff rename destination"))?)?;
                ensure!(first_path != destination, "Git diff rename source equals destination");
                (if kind == b'R' { "renamed" } else { "added" }, Some(first_path), destination)
            }
            _ => bail!("Unsupported Git commit file status"),
        };
        ensure!(seen.insert(file_path.clone()), "Duplicate Git commit file path");
        ensure!(result.len() < MAX_RECORDS, "Too many Git commit files");
        result.push(CommitFileInfo { path: file_path, status: status.to_string(), old_path });
    }
    Ok(result)
}
...
pub fn parse_tree_entry(bytes: &[u8], expected_path: &str) -> Result<Option<TreeEntry>> {
    ...
    ensure!(path(file_path)? == expected_path, "Git tree lookup matched another path");
    ...
    Ok(Some(TreeEntry { oid: ObjectId::parse(fields[2])?, mode, kind, size }))
}
...
pub fn parse_index_entry(bytes: &[u8], expected_path: &str) -> Result<Option<IndexEntry>> {
    ...
    for record in records {
        let (meta, file_path) = entry_fields(record)?;
        ensure!(path(file_path)? == expected_path, "Git index lookup matched another path");
        let fields: Vec<_> = utf8(meta)?.splitn(4, ' ').collect();
        ensure!(fields.len() == 3, "Invalid Git index entry fields");
        ensure!(matches!(fields[2], "0" | "1" | "2" | "3"), "Invalid Git index stage");
        ensure!(fields[2] == "0", "Git index file has unresolved conflict stages");
        ensure!(result.is_none(), "Duplicate Git index entry");
        let mode = mode(fields[0])?;
        ensure!(matches!(mode, 0o100644 | 0o100755 | 0o120000), "Git index entry is not a blob");
        result = Some(IndexEntry { oid: ObjectId::parse(fields[1])?, mode });
    }
    Ok(result)
}
...
pub fn parse_worktrees(bytes: &[u8]) -> Result<Vec<crate::worktree::WorktreeFact>> {
    ...
    ensure!(!result.is_empty() && result.len() <= MAX_WORKTREES, "Invalid Git worktree inventory size");
    let mut width = None;
    for fact in &result {
        validate_absolute_path(fact.path.to_str().ok_or_else(|| anyhow!("Invalid Git worktree path encoding"))?)?;
        if fact.is_bare {
            ensure!(
        ...
        if let Some(branch) = fact.branch_ref.as_deref() {
            ensure!(!GitRef::parse(branch)?.is_remote(), "Worktree has a nonlocal branch ref");
        }
    }
    Ok(result)
}
