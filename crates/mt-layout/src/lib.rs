...
fn salvage_project_layout(value: &Value, stats: &mut SalvageStats) -> Option<SavedProjectLayout> {
    ...
    Some(SavedProjectLayout {
        ...
        selected_terminal_pane_key: salvage_optional_id(
            object.get("selectedTerminalPaneKey"),
            stats,
        ),
        terminal_order: object
            .get("terminalOrder")
            .and_then(Value::as_array)
            .map(|values| {
                values
                    .iter()
                    .filter_map(|value| salvage_optional_id(Some(value), stats))
                    .collect()
            }),
        tabs,
        active_tab_index,
...
mod tests {
    ...
    fn flat_preferences_normalize_without_changing_legacy_records() {
        ...
        assert!(
            !again.repaired,
            "normalized metadata must not churn on reopen"
        );
    ...
    fn absent_or_stale_flat_selection_prefers_the_legacy_owner_not_presentation_order() {
        ...
        for selected in [None, Some(PaneKey::new())] {
            ...
            assert_eq!(
                candidate.selected_terminal_pane_key.as_ref(),
                Some(&keys[2])
            );
    ...
    fn malformed_flat_preferences_survive_typed_read_and_per_record_salvage() {
        ...
        assert_eq!(
            decoded.layout.terminal_order,
            Some(vec![keys[1].clone(), keys[0].clone()])
        );
        assert_eq!(
            decoded.layout.selected_terminal_pane_key.as_ref(),
            Some(&keys[0])
        );
        ...
        assert_eq!(
            ...
            serde_json::to_value(&decoded.layout).unwrap()
        );
    ...
    fn flat_preferences_dual_write_and_follow_the_latest_alias_snapshot() {
        ...
        assert_eq!(
            legacy_json(&store, "latest").as_deref(),
            Some(destination.as_str())
        );
        assert_ne!(
            legacy_json(&store, "first").as_deref(),
            Some(destination.as_str())
        );
        let reconciled = store
            .reconcile_worktree_layouts(&[first.clone(), latest.clone()], 3)
            .unwrap();
        ...
        assert_eq!(
            worktree_json(&store, &latest.worktree_id).as_deref(),
            Some(destination.as_str())
        );
        fs::remove_dir_all(&dir).ok();
    }
