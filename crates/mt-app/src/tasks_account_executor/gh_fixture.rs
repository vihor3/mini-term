...
fn descendant(inherit_pipes: bool) {
    ...
    if !inherit_pipes { command.stdout(Stdio::null()).stderr(Stdio::null()); }
...
fn main() {
    ...
    for key in ["GH_DEBUG", "DEBUG", "GH_HOST", "GH_REPO", "GH_FORCE_TTY", "BASH_ENV", "ENV", "WSLENV"] {
        assert!(std::env::var_os(key).is_none(), "unexpected inherited override");
    }
    ...
    if args.starts_with(&["auth".into(), "status".into()]) {
        ...
        match std::env::var("MT_FIXTURE_ENUM_CASE").as_deref() {
            Ok("none") => { println!("{{\"hosts\":{{}}}}"); return; }
            Ok("unsupported") => { eprintln!("unknown flag: --json {PREFIX}hidden"); std::process::exit(1); }
            Ok("malformed") => { println!("{{{PREFIX}hidden"); return; }
            _ => {}
        }
        let problem = std::env::var("MT_FIXTURE_ENUM_ERROR").unwrap_or_else(|_| "Bad credentials".into());
        println!("{{\"token\":\"{PREFIX}extra\",\"hosts\":{{\"{host}\":[{{\"state\":\"success\",\"active\":true,\"host\":\"{host}\",\"login\":\"Alice\",\"tokenSource\":\"keyring\",\"token\":\"{PREFIX}extra\"}},{{\"state\":\"error\",\"error\":\"{problem}; {PREFIX}hidden\",\"active\":false,\"host\":\"{host}\",\"login\":\"Broken\",\"tokenSource\":\"keyring\"}}]}}}}");
        return;
    }
    ...
    let token = std::env::var("GH_TOKEN").or_else(|_| std::env::var("GH_ENTERPRISE_TOKEN")).unwrap();
    ...
    let host = argument(&args, "--hostname").or_else(|| argument(&args, "--repo").map(|repo| repo.split('/').next().unwrap())).unwrap();
    let variable = if host == "github.com" || host.ends_with(".ghe.com") { "GH_TOKEN" } else { "GH_ENTERPRISE_TOKEN" };
    assert_eq!(std::env::var(variable).unwrap(), token);
    if args.first().map(String::as_str) == Some("api") {
        let after_data = evidence_path("MT_FIXTURE_PHASE", "data-seen").exists();
        let login = if user == "Wrong" || (user == "WrongAfter" && after_data) { "Other" } else { user };
        println!("{{\"login\":\"{login}\"}}");
        return;
    }
    if user == "Leak" || user == "LeakStderr" {
        if user == "Leak" { println!("{{\"secret\":\"{token}\"}}"); }
        else { println!("[]"); }
        eprintln!("{token}");
        return;
    }
    ...
    if user == "Large" {
        std::io::stdout().write_all(&vec![b'x'; 3 * 1024 * 1024]).unwrap();
        return;
    }
    ...
    let item = format!("{{\"number\":{number},\"title\":\"{user}\",\"state\":\"OPEN\",\"author\":{{\"login\":\"{user}\"}},\"labels\":[],\"updatedAt\":\"2026-09-06T01:02:03Z\",\"url\":\"https://{host}/owner/repo/issues/{number}\",\"body\":\"{user}\"}}");
    if detail { println!("{item}"); } else { println!("[{item}]"); }
}
