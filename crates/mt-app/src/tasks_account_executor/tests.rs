...
impl DescendantEvidence {
    ...
    fn configure(&self, command: &mut Command) {
        command.env("MT_FIXTURE_READY", &self.ready)
            .env("MT_FIXTURE_RELEASE", &self.release)
            .env("MT_FIXTURE_MARKER", &self.marker);
    }

    fn cancel_after_start(&self, cancellation: AccountCancellation) -> std::thread::JoinHandle<bool> {
    ...
    fn assert_retired(&self) {
        assert!(self.ready.exists(), "cleanup fixture never started its descendant");
    ...
    fn in_directory(root: &Path) -> Self {
        Self { ready: root.join("ready"), release: root.join("release"), marker: root.join("marker") }
    }
}
...
fn fixture() -> &'static Fixture {
    ...
    FIXTURE.get_or_init(|| {
        ...
        let result = Command::new(std::env::var_os("RUSTC").unwrap_or_else(|| "rustc".into()))
            .arg("--edition=2021").arg(&source).arg("-o").arg(&executable).output().unwrap();
        assert!(result.status.success(), "synthetic gh fixture compilation failed");
...
fn snapshot(path: &Path) -> ProjectExecutionSnapshot {
    ...
    ProjectExecutionSnapshot {
        project_id: "fixture".into(), root_project_id: "fixture".into(),
        worktree_id: WorktreeId::derive(&repo, "fixture", None), execution_host_id: host,
        canonical_path: path.to_str().unwrap().into(), root_source_path: path.to_str().unwrap().into(),
        backend: ExecutionBackend::Local, host_label: "Actions fixture".into(),
    }
}
...
fn selected_reads(host: &str, login: &str) -> [SelectedAccountRequestPlan; 2] {
    let output = CommandOutput {
        stdout: serde_json::to_vec(&serde_json::json!({"hosts": {(host): [{
            ...
            "tokenSource": "keyring"
        }]}})).unwrap(), exit_code: Some(0), ..CommandOutput::default()
    };
    ...
    [
        ...
        SelectedAccountRequestPlan::detail(&known.accounts()[0], &repo, WorkItemKind::Issue, 7).unwrap(),
    ]
}
...
fn contaminate(command: &mut Command) {
    for key in [
        ...
        "GH_DEBUG", "DEBUG", "GH_HOST", "GH_REPO", "GH_FORCE_TTY", "BASH_ENV", "ENV", "WSLENV",
    ] {
        command.env(key, "inherited_fixture_secret");
    }
}

fn native_request(plan: &SelectedAccountRequestPlan, control: &AccountExecutionControl) -> Result<CommandOutput, AccountExecutionError> {
    let fixture = fixture();
    let root = fixture.root.join(uuid::Uuid::new_v4().to_string());
    std::fs::create_dir_all(&root).unwrap();
    process::native_fixture(&snapshot(&root), &plan.data_plan(), Some(plan), control, plan.output_limit(), &|| {
...
fn native_lookup_failures_wrong_proof_and_echoes_are_static_errors() {
    for (login, expected) in [
        ...
        let error = native_request(&selected("github.com", login), &control(Duration::from_secs(10))).unwrap_err();
...
fn native_discovery_and_capability_execute_with_sanitized_environment() {
    ...
    let run = |plan: &CommandPlan| process::native_fixture(
        &snapshot(&fixture.root), plan, None, &control(Duration::from_secs(10)),
        mt_github::COMMAND_OUTPUT_LIMIT, &make_gh,
    ).unwrap();
    ...
    let known = parse_known_accounts("github.com", &run(&known_accounts_plan("github.com").unwrap())).unwrap();
    assert_eq!(known.accounts().len(), 2);
    assert_eq!(known.accounts()[1].problem(), Some(AccountError::AuthenticationFailed));
...
fn native_process_tree_cleans_descendants_on_success_timeout_and_cancel() {
    let fixture = fixture();
    for (login, cancel) in [
        ("Descendant", false), ("PipeDescendant", false), ("Slow", false),
        ("Slow", true), ("LookupSlow", false), ("LookupSlow", true),
    ] {
        ...
        let control = AccountExecutionControl::new(
            ...
            cancellation.clone(), None,
        ).unwrap();
        let canceller = cancel.then(|| evidence.cancel_after_start(cancellation));
        let result = process::native_fixture(
            &snapshot(&fixture.root), &plan.data_plan(), Some(&plan), &control, plan.output_limit(), &|| {
                let mut command = Command::new(&fixture.executable);
                evidence.configure(&mut command);
                command
            },
        );
        if let Some(canceller) = canceller { assert!(canceller.join().unwrap()); }
        if success { assert!(result.is_ok()); }
        else { assert_eq!(result.unwrap_err(), if cancel { AccountExecutionError::Cancelled } else { AccountExecutionError::TimedOut }); }
...
fn native_windows_failed_attachment_reaps_the_suspended_child() {
    let fixture = fixture();
    let marker = fixture.root.join(format!("suspended-{}", uuid::Uuid::new_v4()));
    let mut command = Command::new(&fixture.executable);
...
#[cfg(unix)]
fn envelope_request(plan: &SelectedAccountRequestPlan, control: &AccountExecutionControl, evidence: Option<&DescendantEvidence>) -> Result<CommandOutput, AccountExecutionError> {
    envelope_command(&plan.data_plan(), Some(plan), control, plan.output_limit(), |command| {
        if let Some(evidence) = evidence { evidence.configure(command); }
    })
}

#[cfg(unix)]
fn envelope_command(
    data: &CommandPlan, plan: Option<&SelectedAccountRequestPlan>, control: &AccountExecutionControl,
    limit: usize, configure: impl FnOnce(&mut Command),
) -> Result<CommandOutput, AccountExecutionError> {
    let fixture = fixture();
    let root = fixture.root.join(uuid::Uuid::new_v4().to_string());
    std::fs::create_dir_all(&root).unwrap();
    let mut source = snapshot(&root);
    source.backend = ExecutionBackend::Wsl { distro: "fixture".into() };
...
fn host_envelope_discovery_projects_only_nonsecret_status_and_preserves_categories() {
    ...
    for capability in [AccountCapability::AuthStatusJson, AccountCapability::NamedAccountLookup] {
        let output = envelope_command(&account_capability_plan(capability), None, &control, 65536, |_| {}).unwrap();
        verify_account_capability(capability, &output).unwrap();
        let unsupported = envelope_command(&account_capability_plan(capability), None, &control, 65536,
            |command| { command.env("MT_FIXTURE_UNSUPPORTED", "1"); }).unwrap();
    ...
    for (diagnostic, expected) in [
        ...
        ("keyring access denied", AccountError::CredentialStoreUnavailable),
        ...
        let output = envelope_command(&plan, None, &control, 65536,
            |command| { command.env("MT_FIXTURE_ENUM_ERROR", diagnostic); }).unwrap();
    ...
    let none = envelope_command(&plan, None, &control, 65536,
        |command| { command.env("MT_FIXTURE_ENUM_CASE", "none"); }).unwrap();
    assert!(parse_known_accounts("github.com", &none).unwrap().accounts().is_empty());
    let unsupported = envelope_command(&plan, None, &control, 65536,
        |command| { command.env("MT_FIXTURE_ENUM_CASE", "unsupported"); }).unwrap();
    assert_eq!(parse_known_accounts("github.com", &unsupported).unwrap_err(), AccountError::UnsupportedAuthStatusJson);
    let malformed = envelope_command(&plan, None, &control, 65536,
        |command| { command.env("MT_FIXTURE_ENUM_CASE", "malformed"); }).unwrap_err();
    assert_eq!(malformed, AccountError::MalformedResponse.into());
    let empty_bin = fixture().root.join("no-python");
    std::fs::create_dir_all(&empty_bin).unwrap();
    let missing = envelope_command(&plan, None, &control, 65536,
        |command| { command.env("PATH", &empty_bin); }).unwrap_err();
...
fn host_envelope_executes_sentinels_with_tracing_and_inherited_auth_disabled() {
    for host in ["github.com", "org.ghe.com", "github.example.com"] {
        for login in ["Alice", "Bob", "Rotate"] {
            for plan in selected_reads(host, login) {
                let output = envelope_request(&plan, &control(Duration::from_secs(10)), None).unwrap();
    ...
    for (login, expected) in [
        ...
        for plan in selected_reads("github.com", login) {
            let error = envelope_request(&plan, &control(Duration::from_secs(10)), None).unwrap_err();
            assert_eq!(error, expected);
        }
    }
}
...
fn host_envelope_cancellation_and_timeout_reap_credential_children() {
    for (login, cancel) in [
        ("PipeDescendant", false), ("Slow", false), ("Slow", true),
        ("LookupSlow", false), ("LookupSlow", true),
    ] {
        let evidence = DescendantEvidence::new();
        let cancellation = AccountCancellation::default();
...
fn host_envelope_protocol_and_host_token_routing_fail_closed() {
    for (host, expected) in [("github.com", "GH_TOKEN"), ("org.ghe.com", "GH_TOKEN"), ("ghe.com", "GH_ENTERPRISE_TOKEN"), ("notghe.com", "GH_ENTERPRISE_TOKEN"), ("github.com.example", "GH_ENTERPRISE_TOKEN")] {
        assert_eq!(auth_variable(host), expected);
    }
    for raw in ["{}", "null", "not-json", r#"{"status":"unknown","token":"fixture_credential_Alice"}"#] {
        assert_eq!(decode_host_reply(raw.as_bytes(), 64).unwrap_err(), AccountExecutionError::Protocol);
        assert!(!host_reply_confirms_cleanup(raw.as_bytes()));
    }
    assert_eq!(decode_host_reply(br#"{"status":"helper-unavailable"}"#, 64).unwrap_err(), AccountExecutionError::HostHelperUnavailable);
    assert!(!host_reply_confirms_cleanup(br#"{"status":"cleanup-failed"}"#));
}

#[cfg(unix)]
pub(crate) fn exercise_ssh_fixture(connection: &mt_config::SshConnection, root: &Path, retire: impl Fn()) {
    let bin = root.join("bin");
    std::fs::create_dir_all(&bin).unwrap();
    let gh = bin.join("gh");
    std::fs::copy(&fixture().executable, &gh).unwrap();
    let directory = root.join(format!("tasks ' $(never-executed) {}", uuid::Uuid::new_v4()));
    ...
    let resolved = crate::execution_host::execute_host_command(
        &source, &CommandPlan::new("/bin/sh", ["-c", "command -v gh"]),
        Duration::from_secs(15), 4096,
    ).unwrap();
    assert_eq!(std::str::from_utf8(&resolved.output.stdout).unwrap().trim(), gh.to_str().unwrap());
    let control = AccountExecutionControl::new(Duration::from_secs(15), AccountCancellation::default(),
        resolved.observed_connection_epoch).unwrap();
    for capability in [AccountCapability::AuthStatusJson, AccountCapability::NamedAccountLookup] {
        let result = probe_account_capability(&source, capability, &control);
        result.result.unwrap();
        assert_eq!(result.observed_connection_epoch, resolved.observed_connection_epoch);
    }
    let known = discover_accounts(&source, "github.com", &control);
    assert_eq!(known.observed_connection_epoch, resolved.observed_connection_epoch);
    ...
    std::thread::scope(|scope| {
        ...
        assert!(evidence.ready.exists(), "SSH credential descendant never started");
        retire();
        std::fs::write(directory.join("finish"), b"finish").unwrap();
        let result = worker.join().unwrap();
        assert_eq!(result.result.unwrap_err(), AccountExecutionError::ContextChanged);
        assert_eq!(result.observed_connection_epoch, resolved.observed_connection_epoch);
    });
    evidence.assert_retired();
    let replaced = discover_accounts(&source, "github.com", &control);
    assert_eq!(replaced.result.unwrap_err(), AccountExecutionError::ContextChanged);
    assert!(replaced.observed_connection_epoch.is_some());
    assert_ne!(replaced.observed_connection_epoch, resolved.observed_connection_epoch);
    ...
    for (login, cancel) in [("Slow", false), ("Slow", true), ("LookupSlow", true)] {
        ...
        let cancellation = AccountCancellation::default();
        let control = AccountExecutionControl::new(Duration::from_secs(if cancel { 15 } else { 5 }),
            cancellation.clone(), None).unwrap();
        let canceller = cancel.then(|| evidence.cancel_after_start(cancellation));
        ...
        assert_eq!(result.result.unwrap_err(), if cancel { AccountExecutionError::Cancelled } else { AccountExecutionError::TimedOut });
        evidence.assert_retired();
    }
    retire();
}
...
impl WslFixture {
    fn open() -> Self {
        assert_eq!(std::env::var("GITHUB_ACTIONS").as_deref(), Ok("true"), "Actions-only fixture");
        let required = |key| std::env::var(key).unwrap_or_else(|_| panic!("missing WSL fixture input: {key}"));
        let run_id = required("GITHUB_RUN_ID");
        let run_attempt = required("GITHUB_RUN_ATTEMPT");
        for value in [&run_id, &run_attempt] {
            assert!(!value.is_empty() && value.bytes().all(|byte| byte.is_ascii_digit()), "invalid Actions run identity");
        }
        let repository = required("GITHUB_REPOSITORY");
        assert!(!repository.is_empty(), "missing Actions repository identity");
        let sha = required("GITHUB_SHA");
        assert!(sha.len() == 40 && sha.bytes().all(|byte| byte.is_ascii_hexdigit()), "invalid Actions commit identity");
        let distro = required("MT_TEST_WSL_DISTRO");
        assert_eq!(distro, format!("mt-tasks-{run_id}-{run_attempt}"), "refusing a non-owned distro");
        ...
        assert!(gh_hash.len() == 64 && gh_hash.bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte)),
        ...
        assert!(owner == WslFixtureOwner {
            schema: 1, kind: "mini-term-tasks-wsl".into(), run_id, run_attempt, repository, sha,
        ...
        for (command, expected) in [
            ...
            fixture.require_success(&output);
            assert!(output.stdout == expected.as_bytes(), "WSL resolved a non-fixture executable");
        }
        let output = fixture.command("/usr/bin/sha256sum", &["/usr/local/bin/gh"]);
        fixture.require_success(&output);
        assert!(output.stdout == format!("{gh_hash}  /usr/local/bin/gh\n").as_bytes(), "WSL fixture ELF checksum mismatch");
    ...
    fn command(&self, program: &str, args: &[&str]) -> CommandOutput {
        let result = crate::execution_host::execute_host_command(&self.source,
            &CommandPlan::new(program, args.iter().copied()), Duration::from_secs(5), 4096)
            .unwrap_or_else(|_| panic!("Actions WSL fixture command could not run"));
        assert!(result.observed_connection_epoch.is_none());
        assert!(!result.output.timed_out && !result.output.stdout_truncated && !result.output.stderr_truncated,
            "incomplete WSL fixture command output");
        result.output
    }

    fn require_success(&self, output: &CommandOutput) {
        assert!(output.exit_code == Some(0), "Actions WSL fixture command failed");
    }
    ...
    fn exists(&self, name: &str) -> bool {
        ...
        assert!(matches!(output.exit_code, Some(0 | 1)), "WSL marker presence is indeterminate");
    ...
    fn assert_retired(&self) {
        assert!(self.exists("ready"), "WSL cleanup fixture never started its descendant");
        self.touch("release");
        std::thread::sleep(Duration::from_secs(1));
        assert!(!self.exists("marker"), "WSL request left a live Linux descendant");
    }
}
...
fn tasks_account_executor_wsl_sentinels_cleanup_and_foreground_host() {
    ...
    assert_eq!(discovered.accounts()[1].problem(), Some(AccountError::AuthenticationFailed));
    ...
    for (name, expected) in [
        ("missing-helper", AccountExecutionError::HostHelperUnavailable),
        ("malformed-reply", AccountExecutionError::Protocol),
        ("malformed-enumeration", AccountError::MalformedResponse.into()),
    ] {
        let case = fixture.case();
        case.touch(name);
    ...
    let case = fixture.case();
    assert_eq!(execute_selected_account(&case.source, &selected("github.com", "Large"), &bounded)
        .result.unwrap_err(), AccountError::MalformedResponse.into());
    ...
    for (login, cancel) in [
        ...
        let bounded = AccountExecutionControl::new(
            Duration::from_secs(if success || cancel { 30 } else { 5 }), cancellation.clone(), None,
        ).unwrap();
        let canceller = cancel.then(|| case.cancel_after_start(cancellation));
        let result = execute_selected_account(&case.source, &selected("github.com", login), &bounded);
        if let Some(canceller) = canceller { assert!(canceller.join().unwrap(), "WSL descendant did not become ready"); }
        if success { assert!(result.result.is_ok()); }
        else { assert_eq!(result.result.unwrap_err(), if cancel { AccountExecutionError::Cancelled } else { AccountExecutionError::TimedOut }); }
        case.assert_retired();
    }
}
