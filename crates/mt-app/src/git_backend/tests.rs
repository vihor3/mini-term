...
fn snapshot(path: &str, backend: ExecutionBackend) -> ProjectExecutionSnapshot {
    ...
    ProjectExecutionSnapshot {
        project_id: "fixture-project".into(), root_project_id: "fixture-root".into(),
        worktree_id: WorktreeId::derive(&repo, path, None), execution_host_id: host,
        canonical_path: path.to_owned(), root_source_path: path.to_owned(), backend, host_label: "fixture".into(),
    }
}

fn output(bytes: Vec<u8>, dispatch: Dispatch, code: Option<i32>) -> Attempt {
    Attempt { dispatch, error: None, output: Some(CommandOutput {
        stdout: bytes, stderr: Vec::new(), exit_code: code,
        timed_out: dispatch == Dispatch::Uncertain, stdout_truncated: false, stderr_truncated: false,
    }) }
}
...
struct FakeHost { snapshot: ProjectExecutionSnapshot, state: Arc<Mutex<FakeState>> }

impl Host for FakeHost {
    fn snapshot(&self) -> &ProjectExecutionSnapshot { &self.snapshot }
    fn run(&self, cwd: &str, plan: &GitCommand) -> Attempt {
        ...
        if plan.effect == cli::CommandEffect::Mutation {
            if let Some(lifetime) = &state.invalidate_on_dispatch { lifetime.invalidate(); }
        ...
        let root = if let Some(repositories) = &state.resolver_repositories {
            assert!(repositories.contains(cwd), "resolver sent Git to a non-repository");
            cwd
        } else { &self.snapshot.canonical_path };
        let bytes = if has("--show-toplevel") { format!("{root}\n").into_bytes() }
        else if has("--absolute-git-dir") || has("--git-common-dir") {
            format!("{root}/{}\n", if state.changed_authority { ".other" } else { ".git" }).into_bytes()
        } else if has("status") {
            if state.malformed_status { b"# branch.oid (initial)\0".to_vec() }
            else {
                let mut bytes = format!("# branch.oid {OID}\0# branch.head main\0").into_bytes();
                for name in state.files.keys() { bytes.extend_from_slice(format!("? {name}\0").as_bytes()); }
                bytes
            }
        } else if has("ls-files") { Vec::new() }
        else if has("hash-object") {
            format!("{}\n", state.files.get(plan.args.last().unwrap()).unwrap()).into_bytes()
        } else if has("for-each-ref") { format!("refs/heads/main\0{OID}\0\0\n").into_bytes() }
        else if has("worktree") { format!("worktree {root}\0HEAD {OID}\0branch refs/heads/main\0\0").into_bytes() }
        else { panic!("unexpected read plan: {:?}", plan.args); };
        output(bytes, Dispatch::Completed, Some(0))
    }
    fn canonical_directory(&self, path: &str) -> GitResult<String> { Ok(path.to_owned()) }
    fn kind(&self, path: &str) -> GitResult<Option<NodeKind>> {
        let state = self.state.lock();
        let Some(repositories) = &state.resolver_repositories else { return Ok(Some(NodeKind::Directory)); };
        Ok((state.resolver_directories.contains(path)
            || path.strip_suffix("/.git").is_some_and(|root| repositories.contains(root))).then_some(NodeKind::Directory))
    }
    fn directories(&self, _: &str) -> GitResult<Vec<String>> {
        assert!(self.state.lock().resolver_repositories.is_none(), "file resolver must not scan descendants");
        Ok(Vec::new())
    }
    fn directory_empty(&self, _: &str) -> GitResult<bool> { Ok(true) }
    fn file_kind(&self, _: &str, path: &str) -> GitResult<Option<NodeKind>> { Ok(self.state.lock().files.contains_key(path).then_some(NodeKind::File)) }
    fn read_file(&self, _: &str, path: &str) -> GitResult<OwnedContent> {
        Ok(self.state.lock().files.get(path).map(|text| OwnedContent::Bytes(text.as_bytes().to_vec())).unwrap_or(OwnedContent::Missing))
    }
...
fn fake() -> (GitRepository, Arc<Mutex<FakeState>>) {
    ...
    let snapshot = snapshot(&root, ExecutionBackend::Wsl { distro: "fake-only".into() });
    let state = Arc::new(Mutex::new(FakeState::default()));
    let backend = GitBackend {
        host: Arc::new(FakeHost { snapshot, state: state.clone() }), lifetime: GitLifetime::new(), anchor: root.clone(), probe_anchor: root.clone(), recovery_only: false,
    };
...
fn readiness_epoch_bootstraps_none_but_never_replaces_a_captured_epoch() {
    ...
    assert_eq!(GitBackend::checked_readiness_epoch(Some(7), 8), Err(GitError::stale()));
    assert_eq!(GitBackend::checked_readiness_epoch(Some(8), 7), Err(GitError::stale()));
}
...
fn queued_write_rejects_same_status_changed_bytes_without_dispatch() {
    ...
    state.lock().files.insert("line\n:literal[1]".into(), OID.into());
    let prepared = repository.prepare_write(GitWrite::Discard { paths: vec!["line\n:literal[1]".into()] }).unwrap();
...
fn queued_source_cancellation_and_changed_common_directory_prevent_dispatch() {
    let (repository, state) = fake();
    let prepared = repository.prepare_write(GitWrite::Commit { message: "fixture".into() }).unwrap();
...
fn uncertain_write_outlives_view_and_only_its_id_can_release_quarantine() {
    let (repository, state) = fake();
    let prepared = repository.prepare_write(GitWrite::Commit { message: "one attempt".into() }).unwrap();
    let id = prepared.id();
    ...
    assert!(repository.busy().is_none());
    assert_eq!(state.lock().commands.iter().filter(|args| args.iter().any(|arg| arg == "commit")).count(), 1);
}
...
fn malformed_capture_is_an_error_not_a_clean_repository() {
    ...
    assert!(repository.request(GitRead::Status).unwrap().execute().is_err());
...
fn bounded_capture_rejects_both_streams_and_unconfirmed_completion() {
    ...
    assert!(output(Vec::new(), Dispatch::Uncertain, None).checked(&plan).is_err());
    assert!(output(Vec::new(), Dispatch::NotDispatched, None).checked(&plan).is_err());
}
...
fn file_resolver_obeys_nearest_repository_deleted_parents_and_ancestor_ceiling() {
    ...
    let state = Arc::new(Mutex::new(FakeState {
        resolver_repositories: Some([root.to_owned(), format!("{root}/nested")].into_iter().collect()),
        resolver_directories: ["/source", "/source/a", "/source/a/b", "/source/a/b/c", "/source/a/b/c/d", "/source/a/b/c/d/e", root, "/source/a/b/c/d/e/f/nested"].into_iter().map(str::to_owned).collect(),
        ..FakeState::default()
    }));
    let backend = GitBackend {
        host: Arc::new(FakeHost { snapshot: snapshot(root, ExecutionBackend::Wsl { distro: "fake-only".into() }), state: state.clone() }),
        lifetime: GitLifetime::new(), anchor: root.into(), probe_anchor: root.into(), recovery_only: false,
    };
    let (repository, relative) = backend.repository_for_file("nested/deleted/parent/:literal\\name\n").unwrap();
    assert_eq!(repository.authority().worktree_root, format!("{root}/nested"));
    ...
    assert_eq!(backend.repository_for_file("file").err().unwrap().kind, GitErrorKind::Unavailable);
    ...
    for path in ["../escape", "/absolute", "a/../escape", "a//file", "directory/", ".git/config"] {
...
mod actions_fixtures {
    ...
    struct Fixture { root: PathBuf, snapshot: ProjectExecutionSnapshot }
    impl Fixture {
        fn new(parent: &Path) -> Self {
            let root = parent.join(format!("mt-git-adapter-{}-{}", std::process::id(), next_id().unwrap()));
        ...
        fn git(&self, args: &[&str]) -> Vec<u8> {
            ...
            ExecutionGitHost(self.snapshot.clone()).run(self.root.to_str().unwrap(), &plan).checked(&plan).unwrap()
        }
        fn repository(&self) -> GitRepository {
            GitBackend::connect(self.snapshot.clone(), GitLifetime::new()).unwrap().discover().unwrap().remove(0)
        }
        fn write(&self, name: &str, bytes: &[u8]) { std::fs::write(self.root.join(name), bytes).unwrap(); }
        fn seed(&self) { self.write("tracked", b"head\n"); self.git(&["add", "--", "tracked"]); self.git(&["commit", "-m", "root"]); }
    }
    ...
    impl Host for NoCommands {
        fn snapshot(&self) -> &ProjectExecutionSnapshot { self.0.snapshot() }
        fn run(&self, _: &str, _: &GitCommand) -> Attempt { panic!("native read attempted a host command") }
        fn canonical_directory(&self, path: &str) -> GitResult<String> { self.0.canonical_directory(path) }
        fn kind(&self, path: &str) -> GitResult<Option<NodeKind>> { self.0.kind(path) }
        fn directories(&self, path: &str) -> GitResult<Vec<String>> { self.0.directories(path) }
        fn directory_empty(&self, path: &str) -> GitResult<bool> { self.0.directory_empty(path) }
        fn file_kind(&self, root: &str, path: &str) -> GitResult<Option<NodeKind>> { self.0.file_kind(root, path) }
        fn read_file(&self, root: &str, path: &str) -> GitResult<OwnedContent> { self.0.read_file(root, path) }
        fn remove_file(&self, _: &str, _: &str) -> Attempt { panic!("native read attempted file removal") }
    }

    fn value(repository: &GitRepository, read: GitRead) -> GitReadValue { repository.request(read).unwrap().execute().unwrap().value }
    fn write(repository: &GitRepository, operation: GitWrite) -> GitWriteOutcome {
        ...
        assert!(outcome.succeeded(), "write failed: {:?}; reconciliation: {:?}", outcome.error, outcome.reconciliation_error);
        outcome
    }
    ...
    fn actual_native_read_adapter_never_requires_a_host_git_command() {
        ...
        let backend = GitBackend {
            host: Arc::new(NoCommands(ExecutionGitHost(fixture.snapshot.clone()))), lifetime: GitLifetime::new(),
            anchor: root.clone(), probe_anchor: root, recovery_only: false,
        };
        let repository = backend.discover().unwrap().remove(0);
        let GitReadValue::Status(status) = value(&repository, GitRead::Status) else { panic!() };
        let commit = status.head.oid.unwrap();
        value(&repository, GitRead::Branches);
        value(&repository, GitRead::History { before: None, branch: status.head.branch, limit: 30 });
        value(&repository, GitRead::CommitFiles { commit: commit.clone() });
        value(&repository, GitRead::CommitDiff { commit, path: "tracked".into(), old_path: None });
        value(&repository, GitRead::WorkingDiff { path: "tracked".into(), old_path: None, staged: true });
        value(&repository, GitRead::WorkingDiff { path: "tracked".into(), old_path: None, staged: false });
    }
    ...
    fn actual_native_file_resolver_keeps_nested_deleted_and_working_head_semantics() {
        ...
        let backend = GitBackend {
            host: Arc::new(NoCommands(ExecutionGitHost(fixture.snapshot.clone()))), lifetime: GitLifetime::new(),
            anchor: root.clone(), probe_anchor: root.clone(), recovery_only: false,
        };
        let prefix = nested.root.file_name().unwrap().to_str().unwrap();
        let file = format!("{prefix}/tracked");
        let (repository, relative) = backend.repository_for_file(&file).unwrap();
        assert_eq!(repository.authority().worktree_root, nested.root.to_str().unwrap());
        assert_eq!(relative, "tracked");
        let GitReadValue::Diff(diff) = value(&repository, GitRead::WorkingDiff { path: relative, old_path: None, staged: false }) else { panic!() };
        assert_eq!(diff.old_content, "head\n");
        assert_eq!(diff.new_content, "working\n");
        assert_eq!(serde_json::to_value(&diff).unwrap(), serde_json::to_value(git::get_git_diff(&fixture.root, &file, Some(false)).unwrap()).unwrap());
        let (repository, relative) = backend.repository_for_file(&format!("{prefix}/deleted/parent/file")).unwrap();
        ...
        let state = Arc::new(Mutex::new(FakeState {
            resolver_repositories: Some(BTreeSet::new()), resolver_directories: [root.clone()].into_iter().collect(),
            ..FakeState::default()
        }));
        let remote = GitBackend {
            host: Arc::new(FakeHost { snapshot: snapshot(&root, ExecutionBackend::Wsl { distro: "fake-only".into() }), state: state.clone() }),
            lifetime: GitLifetime::new(), anchor: root.clone(), probe_anchor: root, recovery_only: false,
        };
    ...
    fn actual_detached_native_discovery_preserves_existing_repository_label() {
        ...
        let expected = existing.iter().find(|row| row.path == fixture.root).unwrap();
        assert!(expected.current_branch.as_ref().is_some_and(|label| label.starts_with('(')));
    ...
    fn actual_local_unborn_literal_stage_unstage_discard_and_diff_parity() {
        ...
        write(&repository, GitWrite::Stage { paths: vec![hostile.into()] });
        let GitReadValue::Status(status) = value(&repository, GitRead::Status) else { panic!() };
        assert!(status.head.oid.is_none());
        assert!(status.changes.iter().find(|row| row.path == hostile).unwrap().staged_status.is_some());
        assert!(status.changes.iter().find(|row| row.path == "other").unwrap().staged_status.is_none());
        write(&repository, GitWrite::UnstageAll);
        assert_eq!(std::fs::read(fixture.root.join(hostile)).unwrap(), b"literal\n");
        write(&repository, GitWrite::Discard { paths: vec![hostile.into()] });
        assert!(!fixture.root.join(hostile).exists());
        write(&repository, GitWrite::StageAll);
        write(&repository, GitWrite::Commit { message: "root".into() });
        fixture.write("other", b"working\n");
        let GitReadValue::Diff(diff) = value(&repository, GitRead::WorkingDiff { path: "other".into(), old_path: None, staged: false }) else { panic!() };
    ...
    fn actual_local_deleted_binary_limits_and_symlink_parent_are_explicit() {
        ...
        let GitReadValue::Diff(diff) = value(&repository, GitRead::WorkingDiff { path: "tracked".into(), old_path: None, staged: false }) else { panic!() };
        assert_eq!(diff.old_content, "head\n");
        assert!(diff.new_content.is_empty());
        write(&repository, GitWrite::Discard { paths: vec!["tracked".into()] });
        ...
        assert!(repository.request(GitRead::WorkingDiff { path: "alias/tracked".into(), old_path: None, staged: false }).unwrap().execute().is_err());
    }
    ...
    fn actual_worktree_postconditions_and_discovery_do_not_inject_linked_siblings() {
        ...
        let outcome = write(&repository, GitWrite::WorktreeAdd { target: target.clone(), branch: GitRef::local("topic").unwrap(), create_branch: true, base: None });
        assert!(matches!(outcome.reconciliation.unwrap().postcondition, GitPostcondition::WorktreeCreated { .. }));
        assert_eq!(repository.backend.discover().unwrap().len(), 1);
        let outcome = write(&repository, GitWrite::WorktreeRemove { target: target.clone(), force: false });
        assert!(matches!(outcome.reconciliation.unwrap().postcondition, GitPostcondition::WorktreeRemoved { .. }));
    ...
    fn actual_removing_the_source_worktree_uses_a_verified_survivor_for_reconciliation() {
        ...
        write(&main, GitWrite::WorktreeAdd { target: target.clone(), branch: GitRef::local("source-to-remove").unwrap(), create_branch: true, base: None });
        let mut source = fixture.snapshot.clone();
        source.canonical_path = target.clone();
        let repository = GitBackend::connect(source, GitLifetime::new()).unwrap().discover().unwrap().remove(0);
        let outcome = write(&repository, GitWrite::WorktreeRemove { target: target.clone(), force: false });
        let reconciliation = outcome.reconciliation.unwrap();
        assert!(matches!(reconciliation.postcondition, GitPostcondition::WorktreeRemoved { .. }));
        assert_eq!(reconciliation.repository.authority.common_dir, main.authority.common_dir);
        assert!(reconciliation.repository.request(GitRead::Status).is_err());
        assert!(reconciliation.repository.prepare_write(GitWrite::StageAll).is_err());
        assert!(reconciliation.repository.backend().repository_for_file("tracked").is_err());
        assert!(!Path::new(&target).exists());
    }
    ...
    fn actual_loopback_ssh_git_backend_reads_writes_epoch_and_sftp_containment() {
        ...
        source.backend = ExecutionBackend::Ssh {
            connection_fingerprint: remote_ssh::connection_fingerprint(&connection), connection: connection.clone(), connection_epoch: None,
        };
        let backend = GitBackend::connect(source.clone(), GitLifetime::new()).unwrap();
        let repository = backend.discover().unwrap().remove(0);
        assert!(matches!(backend.snapshot().backend, ExecutionBackend::Ssh { connection_epoch: Some(_), .. }));
        let nested = Fixture::new(&fixture.root);
        nested.seed();
        let prefix = nested.root.file_name().unwrap().to_str().unwrap();
        let (nested_repository, relative) = backend.repository_for_file(&format!("{prefix}/tracked")).unwrap();
        assert_eq!(nested_repository.authority().worktree_root, nested.root.to_str().unwrap());
        assert_eq!(relative, "tracked");
        assert!(nested_repository.matches_snapshot(backend.snapshot()));
        nested.write("literal\\name\n", b"remote literal bytes\n");
        let (nested_repository, relative) = backend.repository_for_file(&format!("{prefix}/literal\\name\n")).unwrap();
        assert_eq!(relative, "literal\\name\n");
        let GitReadValue::Diff(diff) = value(&nested_repository, GitRead::WorkingDiff { path: relative, old_path: None, staged: false }) else { panic!() };
        ...
        write(&repository, GitWrite::Stage { paths: vec![hostile.into()] });
        write(&repository, GitWrite::Unstage { paths: vec![hostile.into()] });
        write(&repository, GitWrite::Discard { paths: vec![hostile.into()] });
        assert!(!fixture.root.join(hostile).exists());
        symlink(&fixture.root, fixture.root.join("alias")).unwrap();
        assert!(repository.request(GitRead::WorkingDiff { path: "alias/tracked".into(), old_path: None, staged: false }).unwrap().execute().is_err());
        write(&repository, GitWrite::Stage { paths: vec!["tracked".into()] });
        write(&repository, GitWrite::Commit { message: "remote host commit".into() });
        ...
        write(&repository, GitWrite::WorktreeAdd { target: target.clone(), branch: GitRef::local("ssh-topic").unwrap(), create_branch: true, base: None });
        write(&repository, GitWrite::WorktreeRemove { target, force: false });
        write(&repository, GitWrite::WorktreePrune);
        let prepared = repository.prepare_write(GitWrite::Commit { message: "must not run".into() }).unwrap();
        remote_ssh::invalidate_connection(&connection.id);
        let outcome = prepared.execute();
        assert_eq!(outcome.state, GitWriteState::NotDispatched);
        assert!(repository.busy().is_none());
        let fresh = GitBackend::connect(source, GitLifetime::new()).unwrap();
        assert_ne!(fresh.snapshot().source_signature(), backend.snapshot().source_signature());
        assert_eq!(GitBackend::connect(backend.snapshot().clone(), GitLifetime::new()).err().unwrap().kind, GitErrorKind::Stale);
        let mut stale_file_anchor = backend.snapshot().clone();
        stale_file_anchor.canonical_path = fixture.root.join("tracked").to_str().unwrap().to_owned();
        // Epoch rejection must precede the non-directory anchor's SFTP error.
        assert_eq!(GitBackend::connect(stale_file_anchor, GitLifetime::new()).err().unwrap().kind, GitErrorKind::Stale);
        let same_epoch = GitBackend::connect(fresh.snapshot().clone(), GitLifetime::new()).unwrap();
        assert_eq!(same_epoch.snapshot().source_signature(), fresh.snapshot().source_signature());
        remote_ssh::invalidate_connection(&connection.id);
    }
    ...
    fn actual_loopback_ssh_dispatched_timeout_keeps_ownership_until_explicit_review() {
        ...
        let repository = backend.discover().unwrap().remove(0);
        let mut prepared = repository.prepare_write(GitWrite::Commit { message: "fixture timeout".into() }).unwrap();
        let id = prepared.id();
        ...
        let pid = std::fs::read_to_string(fixture.root.join("hook-git-pid")).unwrap().parse::<u32>().unwrap();
        let deadline = Instant::now() + Duration::from_secs(15);
        while Path::new("/proc").join(pid.to_string()).exists() {
            assert!(Instant::now() < deadline, "fixture Git process did not stop");
            std::thread::sleep(Duration::from_millis(25));
        }
        repository.review_uncertain(id, UncertainReview::UserConfirmedOriginalOperationStopped).unwrap();
        assert!(repository.busy().is_none());
        remote_ssh::invalidate_connection(&connection.id);
    }
}
