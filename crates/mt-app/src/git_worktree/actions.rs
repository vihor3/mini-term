...
const STALE: &str =
    "Worktree source or dialog changed. Review the current target before trying again.";
...
fn begin_manager_close(view_lifetime: &GitLifetime, is_top: bool) -> bool {
    if !view_lifetime.is_valid() || !is_top {
        return false;
    }
...
fn close_manager(state: &Entity<WorktreeModal>, window: &mut Window, cx: &mut App) {
    if !begin_manager_close(
        &state.read(cx).view_lifetime,
        crate::overlay::is_top(crate::overlay::key(kind::GIT_WORKTREE)),
    ) {
        return;
    }
...
impl Draft {
    fn capture(state: &WorktreeModal, cx: &App) -> Self {
        Self {
            mode: state.mode,
            branch: match state.mode {
                Mode::Existing => state.sel_branch.clone(),
                Mode::New => state.new_branch.read(cx).value().to_string(),
            },
...
fn selected_host_path(backend: &ExecutionBackend, selected: &str) -> Result<String, String> {
    match backend {
        ExecutionBackend::Wsl { distro } => {
            let path = mt_core::parse_wsl_unc(&selected.replace('/', "\\"))
                .ok_or("Select a folder in the captured WSL distribution")?;
            if !path.distro.eq_ignore_ascii_case(distro) {
                return Err("Selected folder belongs to another WSL distribution".into());
            }
        ...
        ExecutionBackend::Local => {
            if mt_core::parse_wsl_unc(&selected.replace('/', "\\")).is_some() {
                return Err("Selected folder belongs to WSL, not the captured local host".into());
            }
...
pub(super) fn browse_destination(state: &Entity<WorktreeModal>, window: &mut Window, cx: &mut App) {
    ...
    let initial = if draft.path.is_empty() {
        snapshot.canonical_path.clone()
    } else if draft.selected.len() == 1 {
        host_parent(&snapshot.backend, &draft.path).to_string()
    } else {
        draft.path.clone()
    };
    ...
    let (host, epoch) = match &snapshot.backend {
        ...
        ExecutionBackend::Local | ExecutionBackend::Wsl { .. } => {
            (ProjectHostSelection::Local, None)
        }
    };
    ...
    crate::remote_directory_picker::open(
        ...
        move |cx| {
            ...
            s.idle(cx)
                && s.picker_request == request
                && s.load_generation == generation
                && Draft::capture(s, cx) == guard_draft
        },
        move |path, window, cx| {
            ...
            let path = match selected_host_path(&snapshot.backend, &path) {
                Ok(path) => path,
                Err(error) => {
                    form_error(&selected_state, error, cx);
                    return;
                }
            };
...
fn verified_postcondition(outcome: &GitWriteOutcome) -> Option<&GitPostcondition> {
    normal_completion(
        ...
        outcome.reconciliation_error.is_some(),
        outcome.lease_retained,
    )
    .then(|| {
        outcome
            .reconciliation
            .as_ref()
            .map(|result| &result.postcondition)
    })
    .flatten()
}
...
fn outcome_current(
    ...
    state.repositories.values().any(|current| {
        current.authority() == expected.authority()
            && current.backend().snapshot().source_signature()
                == expected.backend().snapshot().source_signature()
...
fn register(
    ...
    let s = state.read(cx);
    if !s.is_live(cx)
        || !host_ui::repository_current(repository, &s.store, cx)
        || !target.is_current(
            s.store.read(cx),
            &repository.backend().snapshot().backend,
            path,
            repository.busy().map(|busy| busy.phase),
        )
    {
...
pub(super) fn open_worktree(
    ...
    let Some(repository) = state
        .read(cx)
        .read_repository(group, cx)
        .filter(|repository| repository.busy().is_none())
    else {
        return;
    };
    if !state.read(cx).idle(cx) || !wt.is_valid {
        return;
    }
    let target = match RegistrationTarget::capture(
        state.read(cx).store.read(cx),
        &repository.backend().snapshot().backend,
        &wt.path,
    ) {
        Ok(target) => target,
    ...
    let request = match repository.request(GitRead::Worktrees) {
        Ok(request) => request,
        Err(error) => {
            form_error(state, error.to_string(), cx);
            return;
        }
    };
...
fn create_plans(
    ...
    draft: &Draft,
    cx: &App,
) -> Result<Vec<(GitRepository, GitWrite, Option<RegistrationTarget>)>, String> {
    if draft.path.is_empty() || draft.branch.trim().is_empty() {
        return Err("Choose a branch and worktree destination".into());
    }
    let selected = groups
        .iter()
        .filter(|group| draft.selected.contains(&group.key))
        .collect::<Vec<_>>();
    if selected.is_empty() {
        return Err("Select a repository".into());
    }
    ...
    for group in &selected {
        ...
        if repository.busy().is_some() {
            return Err("A conflicting Git operation is running".into());
        }
        let branches = state
            .branches_by_repo
            .get(&group.key)
            .ok_or("Branches are unavailable")?;
        let branch = match draft.mode {
            Mode::New => GitRef::local(draft.branch.trim()),
            Mode::Existing => {
                let branch = branches
                    .iter()
                    .find(|branch| !branch.is_remote && branch.name == draft.branch)
                    .ok_or("Selected branch changed")?;
        ...
        let base = if draft.mode == Mode::New && !draft.base.is_empty() {
            ...
            if matches.next().is_some() {
                return Err("Base branch name is ambiguous".into());
            }
        ...
        let target = if selected.len() == 1 {
            ...
            host_join(
                ...
                &draft.path,
                &format!("{}-{}", group.name, sanitize_branch_for_dir(&draft.branch)),
            )
        };
        if !targets.insert(host_path_key(&state.snapshot.backend, &target)) {
            return Err("Several repositories resolve to the same destination".into());
        }
        let registration = draft
            .add_as_project
            .then(|| {
                RegistrationTarget::capture(state.store.read(cx), &state.snapshot.backend, &target)
            })
            .transpose()?;
        plans.push((
            repository,
            GitWrite::WorktreeAdd {
                target,
                branch,
                create_branch: draft.mode == Mode::New,
                base,
            },
            registration,
        ));
...
fn prepare_remove(
    ...
    if !removal_live(state.read(cx), form.read(cx), owner, cx) || form.read(cx).removing {
        return;
    }
    let guard =
        project_location(&state.read(cx).snapshot.backend, path).and_then(|(location, _)| {
            state
                .read(cx)
                .store
                .read(cx)
                .prepare_git_worktree_removal(&location, cx)
        });
    let Some((request, force, lifetime)) = form.update(cx, |form, cx| {
        let Some(request) = form.request.checked_add(1) else {
            form.lifetime.invalidate();
            return None;
        };
        ...
        match guard {
            Ok(guard) => {
                form.guard = Some(guard);
                form.error = None;
                form.preparing = true;
            }
            Err(error) => {
                form.error = Some(error);
                form.preparing = false;
                cx.notify();
                return None;
            }
        }
        cx.notify();
        Some((request, form.force, form.lifetime.clone()))
    }) else {
        return;
    };
    let (state, form, expected, path) = (
        state.clone(),
        form.clone(),
        repository.clone(),
        path.to_string(),
    ...
    cx.spawn(async move |cx| {
        let result = cx
            ...
            .spawn(async move {
                ...
                let repository = host_ui::resolve_repository(
                    expected.backend().snapshot().clone(),
                    &expected.authority().worktree_root,
                    lifetime,
                )?;
                if repository.authority() != expected.authority()
                    || repository.backend().snapshot().source_signature()
                        != expected.backend().snapshot().source_signature()
                {
                    return Err(STALE.into());
                }
                repository
                    .prepare_write(GitWrite::WorktreeRemove {
                        target: path,
                        force,
                    })
                    .map_err(|error| error.to_string())
        ...
        let _ = cx.update(|cx| {
            ...
            form.update(cx, |f, cx| {
                ...
                match result {
                    Ok(prepared) => {
                        let valid = host_ui::repository_current(
                            prepared.repository(),
                            &state.read(cx).store,
                            cx,
                        ) && f.guard.as_ref().is_some_and(|guard| {
                            state
                                .read(cx)
                                .store
                                .read(cx)
                                .git_worktree_removal_is_current(guard, cx)
                        });
                        if valid {
                            f.prepared = Some(prepared);
                        } else {
...
pub(super) fn open_remove_confirm(
    ...
    if wt.is_main
        || !wt.is_valid
        || !state.read(cx).idle(cx)
        || crate::prompt::is_open(kind::GIT_WORKTREE_REMOVE)
    {
        return;
    }
    let Some(repository) = state
    ...
    open_guarded_with_close(
        ...
        move |dialog, _, cx| {
            let f = form.read(cx);
            let (force, preparing, removing, error) =
                (f.force, f.preparing, f.removing, f.error.clone());
            let path = f
                .prepared
                .as_ref()
                .and_then(|prepared| match prepared.operation() {
                    GitWrite::WorktreeRemove { target, .. } => Some(target.clone()),
                    _ => None,
                })
                .unwrap_or_else(|| wt.path.clone());
            let (toggle_state, toggle_form, toggle_repository, toggle_path) = (
                state.clone(),
                form.clone(),
                repository.clone(),
                wt.path.clone(),
            ...
            let mut body = div()
                ...
                .flex_col()
                .gap(px(6.0))
                .child(div().text_size(ui::font_px(13.0)).child(tr!(
                    "worktree",
                ...
                .child(
                    div()
                        ...
                        .child(if force { "[x]" } else { "[ ]" })
                        .child(t("worktree", "forceRemove"))
                        .on_click(move |_: &ClickEvent, _, cx| {
                            if toggle_form.read(cx).removing {
                                return;
                            }
                            toggle_form.update(cx, |f, cx| {
                                f.force = !f.force;
                                cx.notify();
                            });
                            prepare_remove(
                ...
                for id in store.project_ids_for_location(&location) {
                    if let Some(project) = store.project(&id) {
                        body = body.child(hint_line(
                            tr!("worktree", "removeAlsoProject", name = project.name.clone()),
                            ui::color_warning(),
                        ));
                    }
                }
            ...
            if let Some(error) = error {
                ...
                let (state, form, repository, path) = (
                    state.clone(),
                    form.clone(),
                    repository.clone(),
                    wt.path.clone(),
                ...
                body = body.child(
                    ui::ghost_button("worktree-revalidate-remove", "Review target").on_click(
                        move |_: &ClickEvent, _, cx| {
                            prepare_remove(&state, &form, &repository, &path, owner, cx)
                        },
                    ),
                );
            }
...
fn remove_worktree(
    ...
    if !removal_live(state.read(cx), form.read(cx), owner, cx)
        || form.read(cx).preparing
        || form.read(cx).removing
    {
        return;
    }
    let store = state.read(cx).store.clone();
    let valid = form
        ...
        .guard
        .as_ref()
        .is_some_and(|guard| store.read(cx).git_worktree_removal_is_current(guard, cx))
        && form.read(cx).prepared.as_ref().is_some_and(|prepared| {
            host_ui::repository_current(prepared.repository(), &store, cx)
                && prepared.repository().busy().is_none()
        });
    if !valid {
        form.update(cx, |f, cx| {
            f.error = Some(STALE.into());
    ...
    let close = store.update(cx, |store, cx| {
        store.close_git_worktree_terminals(guard, source, lifetime, cx)
    });
...
pub(super) fn prune(state: &Entity<WorktreeModal>, group: &RepoGroup, cx: &mut App) {
    ...
    let guards = group
        ...
        .map(|wt| {
            let guard = project_location(&state.read(cx).snapshot.backend, &wt.path).and_then(
                |(location, _)| {
                    let guard = store.read(cx).prepare_git_worktree_removal(&location, cx)?;
                    if !guard.terminals_empty() {
                        return Err(
                            "Terminal records remain; project configuration was kept.".into()
                        );
                    }
                    Ok(guard)
                },
    ...
    cx.spawn(async move |cx| {
        let result = cx
            ...
            .spawn(async move {
                repository
                    .prepare_write(GitWrite::WorktreePrune)
                    .map(|prepared| {
                        (
            ...
            .await;
        let _ = cx.update(|cx| {
            if !owned(state.read(cx), owner, cx) {
                return;
            }
            state.update(cx, |s, cx| {
                s.pruning_key = None;
            ...
            match result {
                ...
                Ok((repository, operation_id, outcome)) => {
                    let current =
                        outcome_current(state.read(cx), &outcome, &repository, operation_id, cx);
                    if let Some(GitPostcondition::WorktreesPruned { paths }) =
                        verified_postcondition(&outcome).filter(|_| current)
                    {
                        for path in paths {
                            match guards.get(path) {
                                Some(Ok(guard)) => {
                                    if let Err(error) = store.update(cx, |store, cx| {
                                        store.finish_git_worktree_removal(guard.clone(), cx)
                                    }) {
                                ...
                                None => errors.push(format!(
                                    "{path}: No captured cleanup authority; configuration was kept."
                                )),
                            }
                        }
                    } else {
                        errors.push(host_ui::outcome_error(&outcome).unwrap_or_else(|| {
                            "Prune was not verified; project configuration was kept.".into()
                        }));
                    }
                }
            }
            if !errors.is_empty() {
                form_error(&state, errors.join("\n"), cx);
            }
            let on_changed = state.read(cx).on_changed.clone();
            on_changed(cx);
            load(&state, cx);
...
pub(super) fn review_uncertain(
    ...
    let Some(repository) = state.read(cx).repositories.get(&group.key).cloned() else {
        return;
    };
    if !state.read(cx).idle(cx)
        || !host_ui::repository_current(&repository, &state.read(cx).store, cx)
    {
        return;
    }
    let state = state.clone();
...
mod tests {
    ...
    fn creation_completion_cannot_register_or_close_over_a_newer_draft() {
        ...
        for current in changed {
            assert!(!original.may_register_completion(&current));
        }
    ...
    fn registration_keeps_captured_alias_order_source_and_uncertain_exclusion() {
        ...
        let source = ExecutionSourceSignature {
            ...
            worktree_id: WorktreeId::derive(&repo, "/repo/feature", None),
        ...
        let alias = |id: &str| RegistrationAlias {
            project_id: id.into(),
            config: serde_json::json!({"id": id, "path": "/configured/alias"}),
            source: Some(source.clone()),
        };
        let original = RegistrationTarget {
            location: ProjectLocationKey::Ssh {
                connection_id: "ssh-a".into(),
                normalized_posix_path: "/repo/feature".into(),
            },
            aliases: vec![alias("first"), alias("second")],
        };
        assert!(original.may_register(&original.clone(), None));
        for phase in [
            GitWritePhase::Validating,
            GitWritePhase::Running,
            GitWritePhase::Reconciling,
            GitWritePhase::Uncertain,
        ] {
        ...
        changed[6].location = ProjectLocationKey::Ssh {
            connection_id: "ssh-b".into(),
            normalized_posix_path: "/repo/feature".into(),
        };
        for current in changed {
            assert!(!original.may_register(&current, None));
        }
        let mut unregistered = original.clone();
        unregistered.aliases.clear();
        assert!(unregistered.may_register(&unregistered, None));
        assert!(
            !unregistered.may_register(&original, None),
            "a later alias cannot replace the original no-target decision"
        );
        let mut canonicalized = unregistered.clone();
        canonicalized.location = ProjectLocationKey::Ssh {
            connection_id: "ssh-a".into(),
            normalized_posix_path: "/elsewhere/feature".into(),
        };
    ...
    fn only_normal_verified_completion_can_authorize_registration_or_cleanup() {
        assert!(normal_completion(
            GitWriteState::Completed,
            false,
            false,
            false
        ));
        for state in [
            GitWriteState::NotDispatched,
            GitWriteState::Completed,
            GitWriteState::Uncertain,
        ] {
            for error in [true, false] {
                for reconciliation_error in [true, false] {
                    for retained in [true, false] {
                        assert_eq!(
                            normal_completion(state, error, reconciliation_error, retained),
                            state == GitWriteState::Completed
                                && !error
    ...
    fn directory_selection_keeps_wsl_distribution_and_local_ownership() {
        ...
        assert_eq!(
            selected_host_path(&wsl, r"\\wsl.localhost\Ubuntu\srv\worktree").unwrap(),
            "/srv/worktree"
        );
        ...
        assert!(
            selected_host_path(&ExecutionBackend::Local, r"\\wsl$\Ubuntu\srv\worktree").is_err()
        );
        assert_eq!(
            selected_host_path(&ExecutionBackend::Local, "/srv/worktree").unwrap(),
            "/srv/worktree"
        );
    }
}
