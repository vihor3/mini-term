...
use mt_github::{
    ...
    parse_known_accounts, require_account_success, verify_account_capability, verify_selected_account,
};
use mt_identity::{ExecutionHostId, HostInstallId, RepoId, WorktreeId};

use super::model::*;
use super::pipeline::*;
use super::service::{apply_detail_completion, apply_list_completion, apply_source_completion, detail_slot_view, invalidates_account, list_request_ids,
    preparation_scope_matches, rotate_scope_state};
use super::{bounded_context, github_project_tasks_enabled_for};
use crate::execution_host::{ExecutionBackend, ExecutionSourceSignature, HostCommandResult, ProjectExecutionSnapshot};
use crate::tasks_account_executor::{AccountCancellation, AccountExecutionControl, AccountExecutionError, AccountHostResult};

fn snapshot(project: &str, path: &str, backend: ExecutionBackend) -> ProjectExecutionSnapshot {
    let install: HostInstallId = "install-v1:00000000-0000-4000-8000-000000000001".parse().unwrap();
    let host = ExecutionHostId::derive("tasks-fixture", &install);
    let repo = RepoId::derive(&host, "/repo/.git");
    ProjectExecutionSnapshot { project_id: project.into(), root_project_id: "root".into(),
        worktree_id: WorktreeId::derive(&repo, path, None), execution_host_id: host,
        canonical_path: path.into(), root_source_path: "/repo".into(), backend,
        host_label: "fixture execution host".into() }
}

fn ssh(epoch: Option<u64>) -> ExecutionBackend {
    ExecutionBackend::Ssh { connection: SshConnection {
        id: "ssh".into(), name: "fixture".into(), host: "example.test".into(), port: 22,
        user: "developer".into(), password: None, identity_file: None, group: None,
    }, connection_fingerprint: 7, connection_epoch: epoch }
}

fn repo() -> GitHubRepoIdentity { GitHubRepoIdentity::new("github.com", "owner", "repo").unwrap() }
fn identity(login: &str) -> GitHubAccountIdentity { GitHubAccountIdentity::new("github.com", login).unwrap() }
fn output(text: impl AsRef<str>) -> CommandOutput {
    CommandOutput { stdout: text.as_ref().as_bytes().to_vec(), exit_code: Some(0), ..Default::default() }
}

fn accounts_output(rows: &[(&str, bool, Option<&str>)]) -> CommandOutput {
    let rows = rows.iter().map(|(login, active, problem)| {
        let mut value = serde_json::json!({"login":login, "active":active,
            "state":if problem.is_some() {"error"} else {"success"}, "host":"github.com"});
        if let Some(problem) = problem { value["error"] = serde_json::json!(problem); }
        value
    }).collect::<Vec<_>>();
...
fn item_output(kind: WorkItemKind, list: bool) -> CommandOutput {
    ...
    output(if list { serde_json::json!([value]).to_string() } else { value.to_string() })
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum Stage { Origin, Capability(AccountCapability), Accounts, Selected(AccountCommandStage) }
...
impl Step {
    fn ok(stage: Stage, output: CommandOutput, epoch: Option<u64>) -> Self {
        Self { stage, result: Ok(output), epoch, cancel: false }
    }

    fn origin(epoch: Option<u64>) -> Self {
        Self::ok(Stage::Origin, output("git@github.com:owner/repo.git\n"), epoch)
    }
    ...
    fn capability(capability: AccountCapability, epoch: Option<u64>) -> Self {
        let flag = if capability == AccountCapability::AuthStatusJson { "--json" } else { "--user" };
        Self::ok(Stage::Capability(capability), output(format!("FLAGS\n  --hostname string\n  {flag} string\n")), epoch)
    }
}
...
impl ScriptedHost {
    fn new(snapshot: &ProjectExecutionSnapshot, steps: Vec<Step>, cancellation: AccountCancellation) -> Self {
        Self { source: snapshot.source_signature(), steps: steps.into(), cancellation,
            last_epoch: None, selected: Vec::new() }
    }

    fn take(&mut self, snapshot: &ProjectExecutionSnapshot, stage: Stage,
        control: Option<&AccountExecutionControl>) -> Step {
        assert_eq!(snapshot.source_signature(), self.source);
        assert!(!self.cancellation.is_cancelled(), "cancelled request dispatched another stage");
        if let Some(control) = control { assert_eq!(control.expected_connection_epoch(), self.last_epoch); }
        let step = self.steps.pop_front().expect("unexpected extra stage");
        assert_eq!(step.stage, stage);
        if step.epoch.is_some() { self.last_epoch = step.epoch; }
        if step.cancel { self.cancellation.cancel(); }
        step
    }

    fn exhausted(&self) { assert!(self.steps.is_empty(), "pipeline skipped a required stage"); }
}

impl TasksExecutor for ScriptedHost {
    fn origin(&mut self, snapshot: &ProjectExecutionSnapshot) -> Result<HostCommandResult, CommandExecutionError> {
        let step = self.take(snapshot, Stage::Origin, None);
        step.result.map(|output| HostCommandResult { output, observed_connection_epoch: step.epoch })
            .map_err(|_| CommandExecutionError::new(CommandExecutionErrorKind::Disconnected, "fixture"))
    }
    ...
    fn capability(&mut self, snapshot: &ProjectExecutionSnapshot, capability: AccountCapability,
        ...
        AccountHostResult { result: step.result.and_then(|output| verify_account_capability(capability, &output).map_err(Into::into)),
    ...
    fn accounts(&mut self, snapshot: &ProjectExecutionSnapshot, host: &str,
        ...
        AccountHostResult { result: step.result.and_then(|output| parse_known_accounts(host, &output).map_err(Into::into)),
            observed_connection_epoch: step.epoch }
    }
    ...
    fn selected(&mut self, snapshot: &ProjectExecutionSnapshot, plan: &SelectedAccountRequestPlan,
        ...
        let result = step.result.and_then(|output| {
            ...
            if plan.stage() == AccountCommandStage::Identity { verify_selected_account(plan.account(), &output)?; }
            Ok(output)
        });
        AccountHostResult { result, observed_connection_epoch: step.epoch }
    }
}

fn preparation_steps(rows: &[(&str, bool, Option<&str>)], epoch: Option<u64>) -> Vec<Step> {
    vec![Step::origin(epoch), Step::capability(AccountCapability::AuthStatusJson, epoch),
        Step::capability(AccountCapability::NamedAccountLookup, epoch), Step::accounts(rows, epoch), Step::origin(epoch)]
}

fn read_steps(kind: WorkItemKind, detail: bool, epoch: Option<u64>) -> Vec<Step> {
    vec![Step::origin(epoch), Step::accounts(&[("Alice", true, None), ("Bob", false, None)], epoch),
        Step::ok(Stage::Selected(if detail {AccountCommandStage::Detail} else {AccountCommandStage::List}),
            item_output(kind, !detail), epoch), Step::origin(epoch),
        Step::accounts(&[("Alice", false, None), ("Bob", true, None)], epoch),
        Step::ok(Stage::Selected(AccountCommandStage::Identity), output(r#"{"login":"Alice"}"#), epoch),
        Step::origin(epoch)]
}

#[test]
fn selection_states_preserve_broken_peers_and_missing_choices() {
    let mixed = accounts(&[("Alice", false, None), ("Bob", true, Some("HTTP 401 bad credentials"))]);
    assert_eq!(mixed.accounts().len(), 2);
    assert_eq!(resolve_selection(&mixed, None), Err(TasksError::ChooseAccount));
    assert_eq!(resolve_selection(&mixed, Some(&identity("alice"))).unwrap().login(), "Alice");
    assert_eq!(resolve_selection(&mixed, Some(&identity("bob"))), Err(AccountError::AuthenticationFailed.into()));
    let solo = accounts(&[("Alice", false, None)]);
    assert_eq!(resolve_selection(&solo, None).unwrap().identity(), &identity("alice"));
    assert_eq!(resolve_selection(&solo, Some(&identity("missing"))), Err(AccountError::SelectedAccountUnavailable.into()));
    assert_eq!(resolve_selection(&accounts(&[]), None), Err(AccountError::AuthRequired.into()));
}
...
fn choices_round_trip_independently_by_root_execution_host_and_github_hostname() {
    ...
    let mut other_root = local.clone(); other_root.root_project_id = "other".into();
    ...
    let wsl = snapshot("wsl", "/repo", ExecutionBackend::Wsl { distro: "Ubuntu".into() });
    let scopes = [account_scope(&local, "github.com"), account_scope(&other_root, "github.com"),
        account_scope(&other_host, "github.com"), account_scope(&remote, "github.com"),
        account_scope(&wsl, "github.com"), account_scope(&local, "github.enterprise.test")];
    assert_eq!(scopes[0], account_scope(&sibling, "github.com"));
    let mut config = AppConfig::default();
    for (index, scope) in scopes.iter().enumerate() {
        write_selection(&mut config, scope.clone(), &GitHubAccountIdentity::new(&scope.github_host, &format!("User{index}")).unwrap());
    }
    ...
    for (index, scope) in scopes.iter().enumerate() {
        assert_eq!(saved_selection(&restored, scope).unwrap().unwrap().login(), format!("user{index}"));
    }
    write_selection(&mut config, scopes[0].clone(), &identity("reselected"));
    assert_eq!(saved_selection(&config, &scopes[1]).unwrap().unwrap().login(), "user1");
    let mut reconnected = remote;
    if let ExecutionBackend::Ssh { connection_epoch, connection_fingerprint, .. } = &mut reconnected.backend {
        *connection_epoch = Some(99); *connection_fingerprint = 42;
    }
    assert_eq!(account_scope(&reconnected, "github.com"), scopes[3]);
    let serialized = serde_json::to_string(&scopes[3]).unwrap();
    for forbidden in ["epoch", "fingerprint", "password", "token", "canonicalPath", "worktreeId"] {
...
fn invalid_or_duplicate_saved_choice_does_not_autoselect() {
    ...
    assert_eq!(saved_selection(&config, &scope), Err(AccountError::InvalidLogin.into()));
    config.tasks_account_selections[0].login = "alice".into();
    config.tasks_account_selections.push(config.tasks_account_selections[0].clone());
    assert_eq!(saved_selection(&config, &scope), Err(AccountError::MalformedResponse.into()));
}

#[test]
fn preparation_and_reads_use_exact_backend_and_ignore_external_active_account_changes() {
    for backend in [ExecutionBackend::Local, ExecutionBackend::Wsl { distro: "Ubuntu".into() }, ssh(Some(9))] {
        let epoch = if matches!(&backend, ExecutionBackend::Ssh { .. }) {Some(9)} else {None};
        let source = snapshot("main", "/repo with ' quote", backend);
        let cancellation = AccountCancellation::default();
        let mut host = ScriptedHost::new(&source, preparation_steps(&[("Alice", false, None), ("Bob", true, None)], epoch), cancellation.clone());
        let prepared = prepare_with(&source, &cancellation, &mut host).result.unwrap();
        assert_eq!(prepared.repository, repo());
        assert_eq!(resolve_selection(&prepared.accounts, Some(&identity("alice"))).unwrap().login(), "Alice");
        host.exhausted();
        for kind in WorkItemKind::ALL {
            for detail in [false, true] {
                let mut host = ScriptedHost::new(&source, read_steps(kind, detail, epoch), cancellation.clone());
                let target = if detail { ReadTarget::Detail(kind, 7) } else { ReadTarget::List(kind) };
                let read = read_with(&source, &repo(), &identity("Alice"), target, &cancellation, &mut host);
                ...
                for plan in &host.selected {
                    ...
                    for forbidden in ["auth switch", "auth login", "--web", "--show-token"] { assert!(!argv.contains(forbidden)); }
                }
                assert!(host.selected[0].data_plan().args.iter().any(|arg| arg == &repo().cli_spec()));
                assert!(host.selected[0].data_plan().args.iter().any(|arg| arg == "--json"));
                host.exhausted();
            }
        }
    }
...
fn first_connection_is_adopted_but_later_success_and_failure_epochs_are_rejected() {
    ...
    let cancellation = AccountCancellation::default();
    let mut host = ScriptedHost::new(&source, preparation_steps(&[("Alice", false, None)], Some(4)), cancellation.clone());
    let ready = prepare_with(&source, &cancellation, &mut host);
    assert!(ready.result.is_ok());
    assert_eq!(ready.observed_source, source.observed_source_signature(Some(4)));
    for failed in [false, true] {
        let mut changed = Step::capability(AccountCapability::AuthStatusJson, Some(5));
        if failed { changed.result = Err(AccountError::Offline.into()); }
        let mut host = ScriptedHost::new(&source, vec![Step::origin(Some(4)), changed], cancellation.clone());
        let result = prepare_with(&source, &cancellation, &mut host);
        assert_eq!(result.result.unwrap_err(), TasksError::changed());
        assert_eq!(result.observed_source, source.observed_source_signature(Some(5)));
        host.exhausted();
    }
}
...
fn account_failure_keeps_its_observed_epoch_and_distinct_category() {
    ...
    for error in [AccountExecutionError::HostHelperUnavailable, AccountError::ClientMissing.into(),
        AccountError::UnsupportedAuthStatusJson.into(), AccountError::UnsupportedNamedAccountLookup.into(),
        AccountError::CredentialStoreUnavailable.into(), AccountError::CredentialLookupFailed.into(),
        AccountError::RateLimited.into(), AccountError::PermissionDenied.into(), AccountExecutionError::TimedOut,
        AccountExecutionError::CleanupFailed, AccountExecutionError::SecretOutputRejected, AccountExecutionError::Protocol] {
        let cancellation = AccountCancellation::default();
        let mut failed = Step::capability(AccountCapability::AuthStatusJson, Some(9));
        failed.result = Err(error);
        let mut host = ScriptedHost::new(&source, vec![Step::origin(Some(9)), failed, Step::origin(Some(9))], cancellation.clone());
        let result = prepare_with(&source, &cancellation, &mut host);
        assert_eq!(result.result.unwrap_err(), TasksError::Account(error));
        assert_eq!(result.observed_source, source.observed_source_signature(Some(9)));
        host.exhausted();
    }
}
...
fn cancellation_before_or_after_origin_never_dispatches_an_account_stage() {
    ...
    for pre_cancelled in [true, false] {
        let cancellation = AccountCancellation::default();
        let steps = if pre_cancelled { cancellation.cancel(); vec![] } else {
            let mut origin = Step::origin(None); origin.cancel = true; vec![origin]
        };
        let mut host = ScriptedHost::new(&source, steps, cancellation.clone());
        let result = prepare_with(&source, &cancellation, &mut host);
        assert_eq!(result.result.unwrap_err(), AccountExecutionError::Cancelled.into());
        assert!(host.selected.is_empty()); host.exhausted();
    }
}
    ...
    for detail in [false, true] {
        for logout in [false, true] {
            ...
            let expected = if logout {
                ...
                steps[5] = Step::ok(Stage::Selected(AccountCommandStage::Identity), output(r#"{"login":"Bob"}"#), None);
                AccountError::WrongHostOrAccount
            };
            let mut host = ScriptedHost::new(&source, steps, cancellation.clone());
            let target = if detail {ReadTarget::Detail(WorkItemKind::Issue, 7)} else {ReadTarget::List(WorkItemKind::Issue)};
            let result = read_with(&source, &repo(), &identity("alice"), target, &cancellation, &mut host);
            assert_eq!(result.result.unwrap_err(), expected.into()); host.exhausted();
        }
    }
}
...
fn list_and_detail_reject_reconnected_data_and_origin_aba() {
    for detail in [false, true] {
        ...
        let result = read_with(&source, &repo(), &identity("alice"), target, &cancellation, &mut host);
        assert_eq!(result.result.unwrap_err(), TasksError::changed());
        assert_eq!(result.observed_source, source.observed_source_signature(Some(10)));
        host.exhausted();

        let mut steps = read_steps(WorkItemKind::Issue, detail, Some(9));
        steps.truncate(4);
        steps[3] = Step::ok(Stage::Origin, output("git@github.com:owner/replacement.git"), Some(9));
        steps.push(Step::origin(Some(9)));
        let mut host = ScriptedHost::new(&source, steps, cancellation.clone());
        let result = read_with(&source, &repo(), &identity("alice"), target, &cancellation, &mut host);
        assert!(matches!(result.result, Err(TasksError::Repository(error)) if error.kind == GitHubErrorKind::RepositoryChanged));
        host.exhausted();
    }
}
...
fn changed_origin_after_account_error_cannot_publish_that_error_for_another_repository() {
    ...
    let mut host = ScriptedHost::new(&source, vec![Step::origin(None), failed,
        Step::ok(Stage::Origin, output("git@github.com:owner/replacement.git"), None)], cancellation.clone());
    let result = prepare_with(&source, &cancellation, &mut host);
    assert!(matches!(result.result, Err(TasksError::Repository(error)) if error.kind == GitHubErrorKind::RepositoryChanged));
    host.exhausted();
}
...
fn each_worktree_proves_its_own_origin_before_sharing_a_data_identity() {
    ...
    let key = |source: &ProjectExecutionSnapshot, repository| RepositoryCacheKey::new(&source.source_signature(), repository, "alice".into(), 7);
    assert_eq!(key(&main, repo()), key(&linked, repo()));
    assert_eq!(key(&main, repo()), RepositoryCacheKey::new(&main.source_signature(), repo(), "Alice".into(), 7));
    let changed = GitHubRepoIdentity::new("github.com", "owner", "different").unwrap();
    assert_ne!(key(&main, repo()), key(&linked, changed));
    assert_ne!(key(&main, repo()), key(&snapshot("ssh", "/repo", ssh(Some(1))), repo()));
}
...
fn ready_source(snapshot: ProjectExecutionSnapshot, generation: u64) -> SourceRecord {
    ...
    source.cache_key = Some(RepositoryCacheKey::new(&snapshot.source_signature(), repo(), "alice".into(), generation));
    source.phase = SourcePhase::Ready;
    source
}

fn owner(source: &SourceRecord, id: u64) -> RequestOwner {
    RequestOwner { source: source.snapshot.source_signature(), source_request: source.request_id,
        request_id: id, scope: source.scope.clone(), cache_key: source.cache_key.clone(),
...
fn scope_rotation_cancels_lists_details_and_sibling_readiness_without_touching_other_choices() {
    ...
    let linked = ready_source(snapshot("linked", "/repo-linked", ExecutionBackend::Local), 7);
    let mut other_snapshot = snapshot("other", "/repo", ExecutionBackend::Wsl {distro:"Ubuntu".into()});
    ...
    let list_owner = owner(&main, 21); let detail_owner = owner(&linked, 22); let other_owner = owner(&other, 23);
    let old_key = main.cache_key.clone().unwrap(); let other_key = other.cache_key.clone().unwrap();
    let mut requests = HashMap::from([(21, list_owner.clone()), (22, detail_owner.clone()), (23, other_owner.clone())]);
    let mut repositories = HashMap::from([(old_key.clone(), RepositoryCache::default()), (other_key.clone(), RepositoryCache::default())]);
    let main_key = main.snapshot.source_signature(); let linked_key = linked.snapshot.source_signature();
    let mut sources = HashMap::from([(main_key.clone(), main), (linked_key.clone(), linked), (other.snapshot.source_signature(), other)]);
    rotate_scope_state(&scope, 30, &mut generations, &mut requests, &mut sources, &mut repositories);
    ...
    assert_eq!(requests.len(), 1); assert!(requests.contains_key(&23));
    assert!(!repositories.contains_key(&old_key)); assert!(repositories.contains_key(&other_key));
...
fn aba_source_account_repository_and_request_generations_cannot_revive_a_completion() {
    ...
    assert!(!owns_request(Some(&source), &owner));
    source.selected = Some(identity("alice"));
    source.cache_key.as_mut().unwrap().repository = GitHubRepoIdentity::new("github.com", "owner", "replacement").unwrap();
    assert!(!owns_request(Some(&source), &owner));
...
fn preparation_fences_known_scope_changes_including_aba_but_not_other_github_hosts() {
    ...
    let mut config = AppConfig::default(); write_selection(&mut config, scope.clone(), &identity("alice"));
    let choices = config.tasks_account_selections.clone();
    let before = HashMap::from([(scope.clone(), 7)]);
    let mut after = before.clone(); after.insert(enterprise, 8);
    assert!(preparation_scope_matches(&scope, &before, &after, &choices, &config.tasks_account_selections));
    write_selection(&mut config, scope.clone(), &identity("bob"));
    assert!(!preparation_scope_matches(&scope, &before, &after, &choices, &config.tasks_account_selections));
    write_selection(&mut config, scope.clone(), &identity("alice")); after.insert(scope.clone(), 9);
    assert!(!preparation_scope_matches(&scope, &before, &after, &choices, &config.tasks_account_selections));
}
...
fn work_item_tabs_preserve_worktree_identity_and_errors_do_not_collapse_into_auth_required() {
    ...
    let key = |worktree_id| GitHubWorkItemTabKey {worktree_id, repository:repo(), kind:WorkItemKind::Issue, number:7};
    assert_ne!(key(main.worktree_id), key(sibling.worktree_id));
    for error in [AccountError::CredentialStoreUnavailable, AccountError::ScopeRequired,
        AccountError::PermissionDenied, AccountError::UnsupportedNamedAccountLookup, AccountError::RateLimited] {
...
fn foreground_list_access_is_consumed_once_and_notifications_cannot_rearm_it() {
    ...
    for phase in [SourcePhase::Loading, SourcePhase::Ready, SourcePhase::Error, SourcePhase::Idle] {
        source.phase = phase;
        for _ in 0..5 { assert_eq!(source.take_list_access(), None); }
    }
...
fn cached_list_foreground_rechecks_logout_and_origin_without_passive_retries() {
    for replaced in [false, true] {
        ...
        let mut slot = ListSlot {
            rows: mt_github::parse_work_item_list(WorkItemKind::Issue,
                &item_output(WorkItemKind::Issue, true)).unwrap(),
            updated_at_unix_ms: Some(1), request_id: 20, ..Default::default()
        };
        source.begin_list_access(30, WorkItemKind::Issue);
        ...
        let steps = if replaced {
            let changed = || Step::ok(Stage::Origin, output("git@github.com:owner/replaced.git"), None);
            vec![changed(), changed()]
        } else {
            vec![Step::origin(None), Step::accounts(&[("Bob", true, None)], None), Step::origin(None)]
        };
        let mut host = ScriptedHost::new(&source.snapshot, steps, access.cancellation.clone());
        let completion = read_with(&source.snapshot, &repo(), &identity("alice"),
            ReadTarget::List(kind), &access.cancellation, &mut host);
        ...
        assert_eq!(apply_source_completion(&mut source, ReadTarget::List(kind), None,
            Some(error.clone())), Some(error));
        assert!(!source_is_ready(Some(&source), &key));
        for _ in 0..5 {
            if let Some((_, kind)) = source.take_list_access() {
                read_with(&source.snapshot, &repo(), &identity("alice"),
                    ReadTarget::List(kind), &access.cancellation, &mut host);
...
fn new_list_access_cancels_only_list_and_preparation_owners_not_detail_accesses() {
    ...
    let requests = HashMap::from([(21, list.clone()), (22, detail.clone()),
        (23, preparation), (24, other)]);
    let key = source.cache_key.clone().unwrap();
    let repositories = HashMap::from([(key, RepositoryCache {
        lists: HashMap::from([(WorkItemKind::Issue, ListSlot {request_id:21, loading:true, ..Default::default()})]),
        details: HashMap::from([((WorkItemKind::Issue, 7), DetailSlot {request_id:22, loading:true, ..Default::default()})]),
    })]);
...
fn cached_detail_slot() -> DetailSlot {
    DetailSlot {
        detail: Some(mt_github::parse_work_item_detail(WorkItemKind::Issue,
            &item_output(WorkItemKind::Issue, false)).unwrap()),
        request_id: 20,
        ..Default::default()
    }
}
...
fn cached_detail_reopen_rejects_logout_through_pipeline_and_scope_invalidation() {
    ...
    let mut host = ScriptedHost::new(&source.snapshot, vec![Step::origin(None),
        Step::accounts(&[("Bob", true, None)], None), Step::origin(None)], access.cancellation.clone());
    let completion = read_with(&source.snapshot, &repo(), &identity("alice"),
        ReadTarget::Detail(WorkItemKind::Issue, 7), &access.cancellation, &mut host);
    ...
    let mut repositories = HashMap::from([(key.clone(), RepositoryCache {
        details: HashMap::from([((WorkItemKind::Issue, 7), slot)]), ..Default::default()
    })]);
    let mut scopes = HashMap::from([(scope.clone(), 7)]);
    rotate_scope_state(&scope, 30, &mut scopes, &mut requests, &mut sources, &mut repositories);
...
fn cached_detail_reopen_revalidates_origin_and_preserves_only_transient_last_known_data() {
    ...
    for replaced in [true, false] {
        ...
        slot.begin_access(access.request_id);
        let steps = if replaced {
            let changed = || Step::ok(Stage::Origin,
                output("git@github.com:owner/replaced.git"), None);
            vec![changed(), changed()]
        } else {
        ...
        assert!(apply_detail_completion(&mut slot, &access, completion.result));
...
fn revalidated_detail_does_not_inherit_an_earlier_source_failure() {
    ...
    let mut host = ScriptedHost::new(&source.snapshot,
        read_steps(WorkItemKind::Issue, true, None), access.cancellation.clone());
    let completion = read_with(&source.snapshot, &repo(), &identity("alice"),
        target, &access.cancellation, &mut host);
    ...
    assert_eq!(apply_source_completion(&mut source, target, Some(accounts), None), None);
    assert!(owns_request(Some(&source), &access));
    let view = detail_slot_view(Some(&slot), &access);
    assert!(view.detail.is_some() && view.error.is_none() && !view.loading);
    assert!(!source_is_ready(Some(&source), access.cache_key.as_ref().unwrap()));
    host.exhausted();
    assert_eq!(apply_source_completion(&mut source, target, None,
        Some(AccountError::NotFound.into())), None);
    assert_eq!(apply_source_completion(&mut source, ReadTarget::List(WorkItemKind::Issue), None,
        Some(AccountError::NotFound.into())), Some(AccountError::NotFound.into()));
    let replaced: TasksError = mt_github::GitHubError::repository_changed().into();
    assert_eq!(apply_source_completion(&mut source, target, None, Some(replaced.clone())), Some(replaced));
}
...
fn detail_access_receipts_reject_reopen_aba_and_never_borrow_another_access_result() {
    ...
    let result = || Ok(ReadSuccess { data: ReadData::Detail(
        mt_github::parse_work_item_detail(WorkItemKind::Issue, &item_output(WorkItemKind::Issue, false)).unwrap()),
    ...
    assert!(!apply_detail_completion(&mut slot, &old, Err(AccountError::Offline.into())));
    assert!(!apply_detail_completion(&mut slot, &current, result()));
