...
fn preparation_source_matches(
    snapshot: &ProjectExecutionSnapshot,
    generations: &HashMap<TasksAccountScope, u64>,
    current_generations: &HashMap<TasksAccountScope, u64>,
    choices: &[TasksAccountSelection],
    current_choices: &[TasksAccountSelection],
) -> bool {
    let execution = account_scope(snapshot, "");
    let same_execution = |scope: &TasksAccountScope| {
        scope.root_project_id == execution.root_project_id
            && scope.execution_host_id == execution.execution_host_id
            && scope.backend == execution.backend
    };
    generations
        .iter()
        .filter(|(scope, _)| same_execution(scope))
        ...
        && choices
            .iter()
            .filter(|entry| same_execution(&entry.scope))
            .eq(current_choices
                .iter()
                .filter(|entry| same_execution(&entry.scope)))
}
...
pub(super) fn invalidates_account(error: &TasksError) -> bool {
    matches!(
        error,
        TasksError::Account(AccountExecutionError::Account(
...
pub(super) fn apply_list_completion(
    slot: &mut ListSlot,
    owner: &RequestOwner,
    result: Result<ReadSuccess, TasksError>,
) -> bool {
    if !owns_slot(slot.request_id, slot.loading, owner) {
        return false;
    }
    slot.loading = false;
    match result {
        Ok(ReadSuccess {
            data: ReadData::List(rows),
            ..
        }) => {
            slot.rows = rows;
            slot.error = None;
...
pub(super) fn apply_detail_completion(
    slot: &mut DetailSlot,
    owner: &RequestOwner,
    result: Result<ReadSuccess, TasksError>,
) -> bool {
...
pub(super) fn apply_source_completion(
    source: &mut SourceRecord,
    target: ReadTarget,
    accounts: Option<mt_github::KnownGitHubAccounts>,
    error: Option<TasksError>,
) -> Option<TasksError> {
    if let Some(accounts) = accounts {
        source.accounts = Some(accounts);
    }
    let invalidated = error
        .as_ref()
        .filter(|error| {
            !error.retains_last_known()
                && !(matches!(target, ReadTarget::Detail(_, _))
                    && matches!(
                        error,
                        TasksError::Account(AccountExecutionError::Account(AccountError::NotFound))
                    ))
        })
        .cloned();
    if matches!(target, ReadTarget::List(_)) {
        source.phase = if error.is_none() {
            SourcePhase::Ready
        } else {
            SourcePhase::Error
        };
...
pub(super) fn detail_slot_view(
    slot: Option<&DetailSlot>,
    access: &RequestOwner,
) -> GitHubDetailView {
    let slot = slot
        .filter(|slot| slot.request_id == access.request_id && !access.cancellation.is_cancelled());
    GitHubDetailView {
        ...
        .filter(|owner| {
            owner.source == *source
                && owner.cache_key.as_ref().is_none_or(|key| {
                    repositories.get(key).is_some_and(|cache| {
                        cache
                            .lists
                            .values()
...
pub(super) fn rotate_scope_state(
    ...
    requests.retain(|_, owner| {
        if owner.scope.as_ref() == Some(scope) {
    ...
    for source in sources
        .values_mut()
        .filter(|source| source.scope.as_ref() == Some(scope))
    {
...
impl GitHubTaskService {
    ...
    fn allocate_id(&mut self) -> u64 {
        ...
        self.next_id = self
            .next_id
            .checked_add(1)
            .expect("Tasks request identity exhausted");
        self.next_id
    }
    ...
    fn cancel_request(&mut self, request_id: u64) {
        let Some(owner) = self.requests.remove(&request_id) else {
            return;
        };
        owner.cancellation.cancel();
    ...
    pub(super) fn reconcile_sources(&mut self, cx: &mut Context<Self>) {
        ...
        for (key, source) in &self.sources {
            ...
            if !current
                .as_ref()
                .is_ok_and(|snapshot| snapshot.source_signature() == *key)
            {
                let initial_connection = source.phase == SourcePhase::Loading
                    && self
                        .requests
                        .values()
                        .any(|owner| owner.source == *key && owner.cache_key.is_none())
            ...
            if source.phase != SourcePhase::Idle
                ...
                && saved_selection(self.store.read(cx).config(), scope)
                    .ok()
                    .flatten()
                    != source.selected
            {
    ...
    pub(super) fn ensure_list(
        ...
        if !current_source_matches(self.store.read(cx), &snapshot, &key) {
            return;
        }
        if force {
        ...
        let Some((access_id, kind)) = self
            .sources
            .get_mut(&key)
            .and_then(SourceRecord::take_list_access)
        else {
            return;
        };
        if self.sources[&key].cache_key.is_some() {
            if self
                .start_read(key.clone(), ReadTarget::List(kind), cx)
                .is_none()
            {
    ...
    pub(super) fn access_list(
        ...
        if !self.sources.contains_key(&key) {
            self.sources
                .insert(key.clone(), SourceRecord::new(snapshot.clone(), access_id));
        }
        self.sources
            .get_mut(&key)
            .expect("access source")
            .begin_list_access(access_id, kind);
    ...
    fn start_preparation(
        ...
        let key = snapshot.source_signature();
        let selected = self.sources[&key].scope.as_ref().and_then(|scope| {
            saved_selection(self.store.read(cx).config(), scope)
                .ok()
                .flatten()
        });
        ...
        cx.spawn(async move |this, cx| {
            ...
            let result = cx
                .background_executor()
                .spawn(async move { prepare_with(&snapshot, &cancellation, &mut HostExecutor) })
                .await;
    ...
    fn finish_preparation(
        ...
        kind: WorkItemKind,
        generations: HashMap<TasksAccountScope, u64>,
        choices: Vec<TasksAccountSelection>,
        completion: PipelineCompletion<PreparedAccounts>,
        cx: &mut Context<Self>,
    ) {
        if !self.requests.contains_key(&owner.request_id) {
            return;
        }
        ...
        let scope_current = match scope.as_ref() {
            ...
            None => preparation_source_matches(
                &snapshot,
                &generations,
                &self.auth_scopes,
                &choices,
        ...
        let Some(mut record) = self.sources.remove(&owner.source) else {
            return;
        };
        // A newer request on the adopted epoch must never be overwritten.
        if self
            .sources
            .get(&completion.observed_source)
            .is_some_and(|other| other.request_id > owner.source_request)
        {
            return;
        }
        ...
        if let Some(scope) = scope.as_ref() {
            ...
            match completion.result {
                Ok(prepared) => {
                    ...
                    let resolved = saved
                        .and_then(|saved| resolve_selection(&prepared.accounts, saved.as_ref()));
                        ...
                        Ok(account) => {
                            if record.selected.is_none() {
                                ...
                                self.store.update(cx, |store, cx| {
                                    store.patch_config(
                                        |config| write_selection(config, scope, &identity),
                                        cx,
                                    );
                                });
                            }
                            record.selected = Some(account.identity().clone());
                            record.cache_key = Some(RepositoryCacheKey::new(
                                &completion.observed_source,
                                prepared.repository,
                                account.identity().login().to_string(),
        ...
        if start_read {
            let _ = self.start_read(source, ReadTarget::List(kind), cx);
        }
        cx.notify();
    }
    ...
    fn start_read(
        ...
        let previous = self
            ...
            .and_then(|cache| match target {
                ReadTarget::List(kind) => cache
                    .lists
                    .get(&kind)
                    .filter(|slot| slot.loading)
                    .map(|slot| slot.request_id),
                ReadTarget::Detail(kind, number) => cache
                    .details
                    .get(&(kind, number))
                    .filter(|slot| slot.loading)
                    .map(|slot| slot.request_id),
        ...
        cx.spawn(async move |this, cx| {
            ...
            let result = cx
                ...
                .await;
            let _ = this.update(cx, |service, cx| {
                service.finish_read(owner, target, result, cx)
            });
        })
        .detach();
        Some(access)
    ...
    fn finish_read(
        ...
        owner: RequestOwner,
        target: ReadTarget,
        completion: PipelineCompletion<ReadSuccess>,
        cx: &mut Context<Self>,
    ) {
        if !self.requests.contains_key(&owner.request_id) {
            return;
        ...
        let Some(cache) = self.repositories.get_mut(cache_key) else {
            return;
        };
        let error = completion.result.as_ref().err().cloned();
        let accounts = completion
            .result
            .as_ref()
            .ok()
            .map(|success| success.accounts.clone());
        match target {
            ReadTarget::List(kind) => {
                let Some(slot) = cache.lists.get_mut(&kind) else {
                    return;
                };
                if !apply_list_completion(slot, &owner, completion.result) {
                    return;
                }
            }
            ReadTarget::Detail(kind, number) => {
                let Some(slot) = cache.details.get_mut(&(kind, number)) else {
                    return;
                };
                if !apply_detail_completion(slot, &owner, completion.result) {
                    return;
                }
            }
        }
    ...
    pub(super) fn select_account(
        ...
        if !github_project_tasks_enabled()
            || snapshot.source_signature() != view.source
        ...
        if source.request_id != view.source_request
            || source.repository != view.repository
            || source.list_access_id != view.list_access_id
            || source.auth_generation != view.auth_generation
            || source
                .accounts
                .as_ref()
                .is_none_or(|accounts| accounts.find(&identity).is_err())
        {
            return;
        }
        let Some(scope) = source.scope.clone() else {
            return;
        };
        if identity.host() != scope.github_host {
            return;
        }
        // An explicitly chosen broken row remains chosen and reports its own error.
        self.store.update(cx, |store, cx| {
            store.patch_config(
                |config| write_selection(config, scope.clone(), &identity),
                cx,
            );
        });
        self.rotate_scope(&scope);
    ...
    pub(super) fn list_view(
        ...
        if let Some(cache_key) = source.cache_key.as_ref()
            && source
                .error
                .as_ref()
                .is_none_or(TasksError::retains_last_known)
            && let Some(slot) = self
                .repositories
                .get(cache_key)
                .and_then(|cache| cache.lists.get(&kind))
        {
            ...
            view.interactive = source_is_ready(Some(source), cache_key)
                && !view.loading
                && view.error.is_none()
                && view.updated_at_unix_ms.is_some();
        }
        view
    }

    pub(super) fn row_is_current(
        &self,
        snapshot: &ProjectExecutionSnapshot,
        view: &GitHubListView,
        kind: WorkItemKind,
        ...
        current_source_matches(self.store.read(cx), snapshot, &view.source)
            ...
            && current.source_request == view.source_request
            && current.list_access_id == view.list_access_id
            && current.repository == view.repository
            && current.account == view.account
            && current.auth_generation == view.auth_generation
    }

    pub(super) fn access_detail(
        &mut self,
        ...
        if !source_has_identity(self.sources.get(&request.source), &key) {
            return None;
        }
        self.start_read(
            request.source,
            ReadTarget::Detail(request.summary.kind, request.summary.number),
            cx,
        )
    }
            ...
            return GitHubDetailView {
                ...
                error: Some(
                    source
                        ...
                        .unwrap_or_else(TasksError::changed),
                ),
            };
        }
        let slot = self.repositories.get(&key).and_then(|cache| {
            cache
                .details
