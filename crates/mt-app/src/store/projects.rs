...
use gpui::Context;
use mt_config::{
    HiddenWorktree, ProjectConfig, WorktreeVisibilityLocation, WorktreeVisibilitySource,
};
...
impl AppStore {
    ...
    pub fn configured_worktree_visibility_key(
        ...
        let path = self
            .trusted_canonical_worktree_path_for_project(project_id)
            .or_else(|| {
        ...
        let path =
            crate::execution_host::configured_execution_path(&snapshot.backend, path).ok()?;
    ...
    pub fn configured_project_visibility_key(
        ...
        let path =
            crate::execution_host::configured_execution_path(&snapshot.backend, &project.path)
                .ok()?;
    ...
    fn apply_project_worktree_visibility(
        ...
        let current_configured_target = |key: &HiddenWorktree| match &key.location {
            WorktreeVisibilityLocation::ConfiguredProject {
                ...
                self.configured_project_visibility_key(
                    &target.root_project_id,
                    configured_project_id,
                )
                .as_ref()
                    == Some(key)
        ...
        if !target.is_current(self)
            ...
            || draft
                .configured_targets()
                .iter()
                .any(|key| !valid_key(key) || !current_configured_target(key))
        {
...
mod project_onboarding_tests {
    ...
    fn visibility_store() -> AppStore {
        ...
        let bindings = HashMap::from([
            (
                a.id.clone(),
                authoritative_remote_binding(&a.id, &a.path, &a.path, &ca),
            ),
            (
                b.id.clone(),
                authoritative_remote_binding(&b.id, &b.path, &b.path, &cb),
            ),
        ]);
    ...
    fn visibility_save_targets_clicked_inactive_project_and_preserves_workbench() {
        ...
        assert!(
            store
                .apply_project_worktree_visibility(&target, &draft)
                .unwrap()
        );
        assert!(
            !store
        ...
        assert!(
            store
                .apply_project_worktree_visibility(&active_target, &active_draft)
                .unwrap()
        );
        assert_eq!(store.active_worktree_id, active);
        assert_eq!(store.active_project_id.as_deref(), Some("a"));
        ...
        for change in 0..6 {
            ...
            match change {
                ...
                4 => {
                    store
                        ...
                        .get_mut("b")
                        .unwrap()
                        .execution_host_id = ExecutionHostId::derive("other", &host_install_id());
                }
                5 => {
            ...
            assert!(
                store
                    ...
                    .projects
                    .iter()
                    .all(|root| root.hidden_worktrees.is_empty())
            );
        }
    }
    ...
    fn visibility_source_uses_offline_authority_but_not_provisional_ssh() {
        ...
        assert_eq!(
            store.configured_worktree_visibility_key("b", "b"),
            Some(key)
        );
        ...
        store
            ...
            .get_mut("b")
            .unwrap()
            .identity_source = WorktreeIdentitySource::ProvisionalSsh.as_str().into();
    ...
    fn visibility_wsl_fallback_reuses_only_previously_saved_exact_paths() {
        ...
        assert!(
            store
                .configured_worktree_visibility_key("wsl", "wsl")
                .is_none()
        );
        ...
        assert_eq!(
            store.configured_worktree_visibility_key("wsl", "wsl"),
            Some(key)
        );
    ...
    fn visibility_wsl_store() -> AppStore {
        ...
        let bindings = projects
            ...
            .map(|project| {
                (
                    project.id.clone(),
                    super::super::identity::binding_from_resolved(
                        project.id.clone(),
                        mt_project::worktree::resolve_provisional_wsl(
                            &host_install_id(),
                            "Ubuntu",
    ...
    fn visibility_wsl_alias_hides_without_resolving_or_merging_canonical_facts() {
        ...
        let alias = groups[0]
            .rows
            .iter()
            .find(|row| row.target.configured_project_id.as_deref() == Some("wsl"))
            .unwrap();
        assert!(alias.visibility_key.is_none());
        assert_eq!(
            alias.configured_visibility_key,
            store.configured_project_visibility_key("wsl", "wsl")
        );
        ...
        assert!(!sidebar_visible(
            alias,
            &store.config.projects[0].hidden_worktrees
        ));
        assert!(sidebar_visible(
            &after[0].rows[0],
            &store.config.projects[0].hidden_worktrees
    ...
    fn visibility_resolved_configured_row_keeps_its_exclusion() {
        ...
        let mut store = visibility_wsl_store();
        let key = store
            .configured_project_visibility_key("wsl", "wsl")
            .unwrap();
        store.config.projects[0].hidden_worktrees.push(key.clone());
        ...
        for canonical in [false, true] {
            for change in 0..5 {
                ...
                let configured = store
                    .configured_project_visibility_key("wsl", "child")
                    .unwrap();
    ...
    fn visibility_saved_only_configured_cleanup_preserves_unrelated_exclusions() {
        for removed in [false, true] {
            ...
            assert_eq!(
                store.apply_project_worktree_visibility(&target, &stale_hide),
                Err("settings.staleSource")
            );

            let mut foreign = unrelated.clone();
            foreign.source.root_path = "/other-root".into();
            store.config.projects[0]
                .hidden_worktrees
                .push(foreign.clone());
            let mut foreign_cleanup =
                VisibilityDraft::new(&store.config.projects[0].hidden_worktrees);
            foreign_cleanup.set_visible(foreign.clone(), true);
            assert_eq!(
                store.apply_project_worktree_visibility(&target, &foreign_cleanup),
                Err("settings.staleSource")
            );
            assert_eq!(
                store.config.projects[0].hidden_worktrees,
                vec![unrelated, foreign]
            );
        }
    }
