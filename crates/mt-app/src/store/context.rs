...
use mt_ai::{
    ...
    AgentProvider, AgentRoute, AgentRuntimeState, sessions::AiSession,
};
use mt_identity::{
...
fn runtime_fallback_title(label: &str, identity: &str) -> String {
    ...
    let identity = identity
        .strip_prefix(AgentRunId::PREFIX)
        .or_else(|| identity.strip_prefix(TerminalSessionId::PREFIX))
        .unwrap_or(identity);
...
fn live_runtime_title(provider: &AgentProvider, title: &str) -> Option<String> {
    let mut title = title
        .trim()
        .trim_start_matches(|ch| ('\u{2800}'..='\u{28ff}').contains(&ch) || ch == '\u{2733}')
        .trim();
...
impl AppStore {
    pub(super) fn clear_runtime_live_title(&mut self, route: &AgentRoute) {
        self.runtime_live_titles
            .retain(|_, title| &title.route != route);
    }
    ...
    pub(super) fn record_runtime_live_title(
        ...
        self.runtime_live_titles.insert(
            run_id.clone(),
            RuntimeLiveTitle {
                route: route.clone(),
                provider: run.provider.clone(),
        ...
        let panel = self
            ...
            .find(|panel| {
                panel
                    .layout
                    .pane(&pane.id)
                    .is_some_and(|current| current.pty_id == Some(pty_id))
            })?;
    ...
    pub fn pane_has_live_agent(&self, project_id: &str, pane: &PaneState) -> bool {
        ...
        match self.current_agent_route_for_pane(project_id, pane) {
            Some(route) if self.agent_runtime.runs().any(|run| &run.route == route) => {
                return super::ai::accepted_agent_projection(&self.agent_runtime, route, false)
                    .live;
            }
            None if pane
                .pty_id
                .is_some_and(|pty_id| self.terminal_routes.contains_key(&pty_id)) =>
            {
                return false;
            }
            _ => {}
    ...
    pub fn pane_agent_provider(&self, project_id: &str, pane: &PaneState) -> Option<String> {
        if pane.pty_id.is_none() {
            return pane
                .shows_ai_session(self.config.ai_auto_resume.unwrap_or(true))
                .then(|| pane.ai_agent())
                .flatten()
                .map(str::to_string);
        }
    ...
    pub(crate) fn apply_runtime_session_titles(
        ...
        self.runtime_session_titles.retain(|run_id, title| {
            self.agent_runtime
                .run(run_id)
                .is_some_and(|run| title.owner.matches(run))
        });
        ...
        for owner in &request.owners {
            if !self
                .agent_runtime
                .run(&owner.run_id)
                .is_some_and(|run| owner.matches(run))
            ...
            if self.runtime_session_titles.get(&owner.run_id) != Some(&title) {
                self.runtime_session_titles
                    .insert(owner.run_id.clone(), title);
                changed = true;
            }
        }
        if changed {
    ...
    fn runtime_pane_title<'a>(
        ...
        let label = run
            .map(|run| run.provider.as_str().to_string())
    ...
    fn resolve_agent_target(&self, run_id: &AgentRunId) -> Option<AgentTargetView> {
        ...
        for project in &self.config.projects {
            ...
            candidates.push(AgentTargetView {
                ...
                activity_freshness: self
                    .agent_runtime
                    .activity_freshness(&run.run_id, chrono::Utc::now().timestamp_millis()),
            ...
            .filter(|project| project.id == project_id)
        {
            ...
            for (panel_index, panel) in state.panels.iter().enumerate() {
                ...
                for pane in panel.layout.panes() {
                    ...
                    let display_status = pane
                        .pty_id
                        .filter(|pty_id| !self.is_pty_exited(*pty_id))
                        .and_then(|_| self.current_agent_route_for_pane(&project.id, pane))
                        .and_then(|route| {
                            super::ai::agent_display_status(
                                &self.agent_runtime,
                                route,
    ...
    pub(super) fn remove_agent_runtime_route(&mut self, route: &AgentRoute) {
        self.runtime_session_titles
            .retain(|_, title| &title.owner.route != route);
            ...
            for panel in &state.panels {
                for pane in panel.layout.panes() {
                    ...
                    let mut agents = self
                        ...
                        .runs()
                        .filter(|run| {
                            route.as_ref() == Some(&run.route) && !run.activity.is_ended()
                        })
                        .filter_map(|run| self.resolve_agent_target(&run.run_id))
                        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn title_session(id: &str, provider: &str, title: &str) -> AiSession {
        AiSession {
            id: id.into(),
            session_type: provider.into(),
            title: title.into(),
            timestamp: "2026-09-06T00:00:00Z".into(),
            model: None,
            wsl_distro: None,
            ssh_connection_id: Some("ssh".into()),
        }
    }
    ...
    fn runtime_title_requires_exact_provider_session_and_execution_source() {
        ...
        assert_eq!(
            session_title_for_owner(&owner, &source, &sessions).as_deref(),
    ...
    fn terminal_title_cardinality_ignores_superseded_alias_and_drops_ended_hook_title() {
        ...
        let mt_ai::AgentApplyOutcome::Applied { run_id: alias, .. } = registry.observe(weak) else {
            panic!("weak rejected");
        };
        ...
        let selected =
            super::super::ai::single_live_agent_for_route(&registry, &run.route).unwrap();
        let owner = RuntimeTitleOwner::from_run(selected).unwrap();
        let title = session_title_for_owner(
            &owner,
            &source,
            &[title_session("exact-session", "codex", "Exact title")],
        );
        assert_eq!(
            runtime_display_title(
                None,
        ...
        let fallback = runtime_display_title(
            None,
            None,
            None,
            "shell",
            run.route.terminal_session_id.as_str(),
        );
    ...
    fn runtime_title_prefers_manual_names_and_bounds_distinguishable_fallbacks() {
        ...
        assert_eq!(
            runtime_display_title(None, None, Some("Live"), "ssh", "terminal1"),
            "Live"
        );
        let first_id = format!(
        ...
        assert_eq!(
            usable_runtime_title(&"x".repeat(200)).unwrap().len(),
            RUNTIME_TITLE_LIMIT
        );
    ...
    fn live_runtime_titles_exclude_provider_status_and_cwd_labels() {
        ...
        assert_eq!(
            live_runtime_title(&claude, "\u{2733} Fix ownership").as_deref(),
            Some("Fix ownership")
        );
        let opencode = "opencode".parse().unwrap();
        assert_eq!(
            live_runtime_title(&opencode, "\u{280b} OC | Review changes").as_deref(),
            Some("Review changes")
        );
        let pi = "pi".parse().unwrap();
        assert!(live_runtime_title(&pi, "\u{03c0} > /repo").is_none());
        assert_eq!(
            live_runtime_title(&pi, "\u{03c0} : Fix tests").as_deref(),
            Some("Fix tests")
        );
    }
    ...
    fn acknowledged_waiting_and_done_are_not_renewed_by_unchanged_process_inventory() {
        for activity in [
            ...
            let outcome = registry.observe(mt_ai::AgentObservation {
                event_id: run.last_event_id.clone(),
                route: run.route.clone(),
                sequence: 1,
                connection_epoch: Some(1),
                provider: run.provider.clone(),
            ...
            let mt_ai::AgentApplyOutcome::Applied { run_id, .. } = outcome else {
                panic!("Hook rejected");
            };
            ...
            for sequence in 2..=4 {
                ...
                let projection =
                    super::super::ai::accepted_agent_projection(&registry, &run.route, false);
                assert!(!agent_event_is_unread(
                    Some(&acknowledged),
