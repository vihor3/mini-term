...
pub(super) fn agent_display_status(
    ...
    live_agent_runs_for_route(registry, route)
        .map(|run| {
            if matches!(
                run.activity,
...
fn activity_for_session_identity(
    ...
    let exact = registry
        .runs()
        .filter(eligible)
        .filter(|run| run.provider_session_id.as_deref() == Some(session_id))
        .collect::<Vec<_>>();
    if !exact.is_empty() {
...
pub(crate) fn record_runtime_status_change(
    ...
    if let Some(session) = exact_session {
        ...
        let Some(provider) = provider.filter(|_| valid_session) else {
            return Some(AgentApplyOutcome::Ignored(
                mt_ai::AgentObservationIgnored::UnresolvedHookOwner,
            ));
        };
        ...
        if session.lifecycle_id.is_none()
            && activity.is_ended()
            && registry
                .runs()
                .filter(|run| {
                    run.route == route
                        ...
                        && run.evidence == AgentEvidence::Hook
                        && !run.activity.is_ended()
                        && run.confirmation == AgentConfirmation::LiveConfirmed
                })
                .take(2)
    ...
    let Some(provider) = provider else {
        return registry
            ...
            .then_some(AgentApplyOutcome::Ignored(
                mt_ai::AgentObservationIgnored::AmbiguousRun,
            ));
    };
    if !hook_event
        && registry
...
fn record_runtime_session_identity(
    ...
    match identity.hook_lifecycle {
        Some(mt_ai::HookLifecycleEvent::Started(id)) => {
            registry.start_hook_lifecycle(observation, id)
        }
        Some(mt_ai::HookLifecycleEvent::Observed(id)) => {
            registry.observe_hook_lifecycle(observation, id)
        }
        None => registry.observe(observation),
    }
}
    ...
    pub fn find_live_session_pane(&self, session_id: &str) -> Option<(String, String, PaneStatus)> {
        ...
        for (project_id, state) in self.project_states.iter() {
            for pane in state.all_panes() {
                ...
                let has_rich_route = self
                    .current_agent_route_for_pane(project_id, pane)
...
mod route_tests {
    ...
    fn apply_source_event(
        ...
        match event {
            ...
            AiEvent::Session {
                ...
                event_id,
                sequence,
            } => record_runtime_session_identity(
                registry,
                route.clone().unwrap(),
                event_id.clone(),
                *sequence,
    ...
    fn source_weak_input(perception: &mt_ai::AiPerception) {
        perception.emitter().emit_if_changed_with_episode(
            ...
            "ai-idle",
            None,
            perception.tracker().ai_session_agent(7),
    ...
    fn producer_bridge_same_id_resume_creates_new_run_for_sole_and_nonlast_sessions() {
        for sibling in [false, true] {
            for detected in [false, true] {
                ...
                let old = registry
                    .runs()
                    .find(|run| run.provider_session_id.as_deref() == Some("resumed"))
                    .unwrap()
                    .run_id
                    .clone();
                ...
                let sibling_before = registry
                    .runs()
                    .find(|run| run.provider_session_id.as_deref() == Some("sibling"))
                    .cloned();
                if sibling {
                ...
                let Some(mt_ai::HookLifecycleEvent::Started(lifecycle_id)) =
                    identity.hook_lifecycle
                else {
                ...
                let AgentApplyOutcome::Applied {
                    run_id: resumed,
                    created: true,
                } = apply_source_event(&mut registry, &first, None)
                else {
                ...
                assert_eq!(
                    apply_source_event(&mut registry, &status, None),
                ...
                assert!(
                    ...
                    "duplicate active start must reuse dedup and identity"
                );
                assert_eq!(registry.run(&resumed), Some(&before_duplicate));
                source_hook(&perception, "UserPromptSubmit", Some("resumed"));
                apply_source_channel(&mut registry, &mut receiver);
                assert_eq!(
                    registry.run(&resumed).unwrap().activity,
                ...
                assert_eq!(
                    live_agent_runs_for_route(&registry, &route).count(),
                    if sibling { 2 } else { 1 }
                );
                source_hook(&perception, "SessionEnd", Some("resumed"));
                apply_source_channel(&mut registry, &mut receiver);
    ...
    fn producer_bridge_queued_old_lifecycle_events_cannot_mutate_a_resumed_run() {
        ...
        assert!(matches!(
            apply_source_event(&mut registry, &ended, None),
            AgentApplyOutcome::Applied { .. }
        ));
        source_hook(&perception, "SessionStart", Some("same"));
        source_hook(&perception, "UserPromptSubmit", Some("same"));
        ...
        for event in [&queued_identity, &queued_work] {
            assert_eq!(
                apply_source_event(&mut registry, event, Some(99)),
                AgentApplyOutcome::Ignored(mt_ai::AgentObservationIgnored::EndedRun)
            );
        }
        assert_eq!(
            apply_source_event(&mut registry, &ended, Some(99)),
        ...
        let AiEvent::Status { change, .. } = &ended else {
            panic!("missing exact end");
        };
        let outcome = record_runtime_status_change(
            &mut registry,
            route.clone(),
            AgentEventId::new(),
            99,
        ...
        assert!(
            matches!(current[0].1, AgentApplyOutcome::Applied { .. }),
            "rejected epoch must not fence current events"
        );
        assert_eq!(
            live_agent_runs_for_route(&registry, &route)
                .find(|run| run.provider_session_id.as_deref() == Some("same"))
                .unwrap()
                .activity,
            AgentActivity::Done
        );
    ...
    fn producer_bridge_rejected_start_does_not_bind_or_consume_its_event() {
        ...
        assert!(matches!(
            apply_source_event(&mut registry, &queued_end, None),
            AgentApplyOutcome::Applied { .. }
        ));
        assert!(matches!(
            apply_source_event(&mut registry, &early_start, None),
            AgentApplyOutcome::Applied { created: true, .. }
        ));
        assert!(matches!(
            apply_source_event(&mut registry, &early_status, None),
    ...
    fn producer_bridge_first_explicit_start_after_mid_session_recognition_can_resume() {
        ...
        let AiEvent::Session { identity, .. } = &observed else {
            panic!("missing mid-session identity");
        };
        let Some(mt_ai::HookLifecycleEvent::Observed(lifecycle_id)) = identity.hook_lifecycle
        else {
        ...
        for event in [&observed, &status] {
            assert_eq!(
                apply_source_event(&mut registry, event, None),
                AgentApplyOutcome::Ignored(mt_ai::AgentObservationIgnored::EndedRun)
            );
        }
        ...
        let AiEvent::Session { identity, .. } = &started else {
            panic!("first explicit start must emit identity");
        };
        assert_eq!(
            identity.hook_lifecycle,
            Some(mt_ai::HookLifecycleEvent::Started(lifecycle_id))
        );
        assert_eq!(
            identity.weak_episode, None,
            "start authority cannot recapture the receipt"
        );
        assert!(matches!(
            apply_source_event(&mut registry, &started, None),
            AgentApplyOutcome::Applied { created: true, .. }
        ));
        apply_source_channel(&mut registry, &mut receiver);
        source_hook(&perception, "SessionStart", Some("same"));
        ...
        assert_eq!(
            perception.tracker().ai_session_agent(7).as_deref(),
            Some("claude")
        );
    }
    ...
    fn producer_bridge_resumed_hook_never_borrows_later_provider_input() {
        for pending in [false, true] {
            ...
            let newer_run = if pending {
                ...
                registry
                    .runs()
                    .find(|run| run.provider.as_str() == "claude")
                    .cloned()
            };
            for event in [
                "UserPromptSubmit",
                ...
                assert!(
                    emitted
                        .iter()
                        .all(|(_, result)| matches!(result, AgentApplyOutcome::Applied { .. }))
                );
            ...
            for inner_first in [false, true] {
                ...
                for sid in ["outer", "inner"] {
                    ...
                    assert_eq!(
                        emitted.len(),
                        2,
                        "same status/cause must not dedup a different Hook sid"
                    );
                    assert!(
                        emitted.iter().all(|(_, outcome)| matches!(
                            outcome,
                            AgentApplyOutcome::Applied { .. }
                        ))
                    );
                ...
                assert_eq!(
                    registry
                        .runs()
                        .filter(|run| run.evidence == AgentEvidence::Hook
                            && run.activity == AgentActivity::Working)
                        .count(),
                    2
                );
                perception.observe_input(7, b"\x04");
                ...
                for (index, sid) in endings.into_iter().enumerate() {
                    ...
                    assert!(repeated.iter().all(|(change, _)| change.weak_episode
                        == if sid == "outer" { original } else { None }));
                    source_hook(&perception, "SessionEnd", Some(sid));
                    ...
                    assert_eq!(
                        registry
                            .runs()
                            .find(|run| run.provider_session_id.as_deref() == Some(sid))
                            .unwrap()
                            .activity,
                        AgentActivity::Exited
                    );
    ...
    fn exact_hook_exit_requires_existing_unique_session_before_side_effects() {
        let route = route();
        let mut registry = AgentRuntimeRegistry::default();
        let mut known = status_observation(
            route.clone(),
            1,
            AgentActivity::Working,
            AgentEvidence::Hook,
    ...
    fn producer_bridge_all_hook_exits_allow_new_input_without_old_alias_resurrection() {
        for inner_first in [false, true] {
            ...
            for _ in 0..3 {
                perception
                    .emitter()
                    .emit_if_changed_with_episode(7, "idle", None, None, original);
            }
            assert!(apply_source_channel(&mut registry, &mut receiver).is_empty());
            ...
            let newer = live_agent_runs_for_route(&registry, &route)
                .next()
                .unwrap()
                .clone();
            ...
            assert_eq!(
                queued[0].1,
                AgentApplyOutcome::Ignored(mt_ai::AgentObservationIgnored::SupersededWeakEpisode)
            );
            assert_eq!(registry.run(&newer.run_id), Some(&newer));
            assert_eq!(live_agent_runs_for_route(&registry, &route).count(), 1);
        }
    ...
    fn accepted_weak_status_preserves_hook_semantics_and_attention() {
        for (activity, status, attention) in [
            ...
            let mut hook = status_observation(route.clone(), 1, activity, AgentEvidence::Hook);
    ...
    fn terminal_display_stops_stale_work_without_losing_agent_liveness() {
        ...
        let AgentApplyOutcome::Applied { run_id, .. } = registry.observe(observation) else {
            panic!("process rejected");
        };
        assert_eq!(
            agent_display_status(&registry, &route, 1),
            Some(PaneStatus::Idle)
        );
        assert!(matches!(
            registry.observe_semantic(mt_ai::AgentSemanticObservation {
                event_id: AgentEventId::new(),
                run_id,
                route: route.clone(),
                provider: "codex".parse().unwrap(),
        ...
        assert_eq!(
            agent_display_status(&registry, &route, 2),
            Some(PaneStatus::AiWorking)
        );
        let expired = 3 + mt_ai::AGENT_SEMANTIC_MAX_AGE_MS;
        assert_eq!(
            agent_display_status(&registry, &route, expired),
            Some(PaneStatus::Idle)
        );
        ...
        registry
            .mark_connectivity(mt_ai::AgentConnectivityObservation {
                event_id: AgentEventId::new(),
                route: route.clone(),
                sequence: 4,
                connection_epoch: Some(1),
    ...
    fn session_identity_never_borrows_newest_other_provider_or_session_activity() {
        ...
        assert_eq!(
            activity_for_session_identity(&registry, &route, &provider, "new"),
            AgentActivity::Unknown
        );
        let mut owned = status_observation(
            route.clone(),
            1,
        ...
        let mut other = status_observation(
            route.clone(),
            2,
            AgentActivity::Working,
            AgentEvidence::Hook,
        ...
        assert_eq!(
            activity_for_session_identity(&registry, &route, &provider, "owned"),
            AgentActivity::Waiting
        );
        assert_eq!(
            activity_for_session_identity(&registry, &route, &provider, "new"),
    ...
    fn session_identity_does_not_promote_unbound_process_semantics_into_hook_authority() {
        ...
        let AgentApplyOutcome::Applied { run_id, .. } = registry.observe(observed) else {
            panic!("process rejected");
        };
        let provider: AgentProvider = "codex".parse().unwrap();
        assert!(matches!(
        ...
        assert_eq!(
            agent_display_status(&registry, &route, 20_000),
            Some(PaneStatus::Idle)
        );
    }
    ...
    fn session_identity_can_retain_a_unique_unbound_hook_activity() {
        ...
        assert_eq!(
            activity_for_session_identity(&registry, &route, &"codex".parse().unwrap(), "new"),
            AgentActivity::Waiting
        );
    }
    ...
    fn unbound_hook_and_two_same_provider_processes_keep_independent_app_liveness() {
        ...
        registry
            .apply_process_inventory(mt_ai::AgentProcessInventoryObservation {
                ...
                processes: [42, 43]
                    .into_iter()
                    .map(|pid| mt_ai::AgentProcessObservation {
                        provider: "codex".parse().unwrap(),
        ...
        assert_eq!(
            registry.runs().filter(|run| run.process.is_some()).count(),
            2
        );
        let projection = accepted_agent_projection(&registry, &route, false);
        assert!(projection.live);
    ...
    fn monitor_silence_is_not_hook_evidence_and_provider_fallback_is_unambiguous() {
        ...
        assert_eq!(
            unique_route_provider(&registry, &route).unwrap().as_str(),
            "codex"
        );
        let outcome = registry.observe(status_observation(
            route.clone(),
        ...
        assert_eq!(
            outcome,
            AgentApplyOutcome::Ignored(mt_ai::AgentObservationIgnored::AmbiguousRun)
        );
        assert!(
            project_status_observation(
                &registry,
        ...
        assert_eq!(
            accepted_agent_projection(&registry, &route, false).status,
    ...
    fn weak_fallback_projection_is_sticky_after_hook_end_but_not_a_later_launch() {
        for hook_first in [false, true] {
            ...
            let mut hook =
                status_observation(route.clone(), 2, AgentActivity::Done, AgentEvidence::Hook);
            ...
            let alias = if hook_first {
                ...
                assert_eq!(
                    ...
                    AgentApplyOutcome::Ignored(
                        mt_ai::AgentObservationIgnored::SupersededWeakEpisode
                    )
                );
                None
            } else {
                let AgentApplyOutcome::Applied { run_id, .. } = registry.observe(weak.clone())
                else {
                    panic!("weak rejected");
                };
                assert!(accepted_agent_projection(&registry, &route, false).live);
                assert_eq!(
                    single_live_agent_for_route(&registry, &route)
                        .unwrap()
                        .run_id,
                    run_id
                );
            ...
            assert_eq!(live_agent_runs_for_route(&registry, &route).count(), 1);
            let hook_id = single_live_agent_for_route(&registry, &route)
                .unwrap()
                .run_id
                .clone();
            let projection = accepted_agent_projection(&registry, &route, false);
            ...
            assert_eq!(projection.provider.as_deref(), Some("codex"));
            assert_eq!(
                agent_display_status(&registry, &route, 3),
                Some(PaneStatus::AiIdle)
            );
            ...
            assert!(matches!(
                registry.observe_hook_exit(route.clone(), AgentEventId::new(), 3, None, 3),
                AgentApplyOutcome::Applied { .. }
            ));
            for sequence in 4..=6 {
    ...
    fn weak_supersession_never_hides_proved_same_provider_runs_or_another_route() {
        ...
        for route in [route.clone(), other_route.clone()] {
            let mut weak =
                status_observation(route, 1, AgentActivity::Unknown, AgentEvidence::PtyActivity);
        ...
        registry
            .apply_process_inventory(mt_ai::AgentProcessInventoryObservation {
                ...
                sequence: 3,
                connection_epoch: 1,
                weak_episode: episode,
                received_at_unix_ms: 3,
                processes: [10, 20]
                    .into_iter()
                    .map(|pid| mt_ai::AgentProcessObservation {
                        provider: "codex".parse().unwrap(),
                        process: mt_ai::AgentProcessIdentity::new(pid, 100).unwrap(),
                        activity: AgentActivity::Unknown,
                    })
                    .collect(),
            })
        ...
        assert_eq!(
            live_agent_runs_for_route(&registry, &other_route).count(),
            1
        );
        assert!(single_live_agent_for_route(&registry, &route).is_none());
        assert!(
            registry
                .runs()
                .filter(|run| run.process.is_some() || run.evidence == AgentEvidence::Hook)
        ...
        assert!(!accepted_agent_projection(&registry, &route, false).live);
        assert_eq!(
            accepted_agent_projection(&registry, &route, false).provider,
            None
        );
    ...
    fn later_different_provider_episode_survives_older_hook_capture_and_retirement() {
        ...
        let AgentApplyOutcome::Applied {
            ...
            panic!("first weak input rejected");
        };
        let mut hook = status_observation(
            route.clone(),
            2,
        ...
        let AgentApplyOutcome::Applied {
            ...
            panic!("later detection rejected");
        };
        ...
        let mut covering =
            status_observation(route.clone(), 7, AgentActivity::Done, AgentEvidence::Hook);
